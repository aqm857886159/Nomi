...
## 0.4 2026-08-29 执行流程 v3（防止 reviewer 逐轮补洞和无信息重跑）
...
10. **相同失败有停止条件。** 同一命令、同一失败签名在源码、fixture、依赖和相关环境均未变化时最多执行两次；第二次后必须先归类为 contract / lifecycle / implementation / environment 并改变对应证据，禁止第三次原样重跑，也禁止用更宽测试诊断已知窄问题。
