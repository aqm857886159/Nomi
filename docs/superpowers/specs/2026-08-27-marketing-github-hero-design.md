...
首屏保留下载按钮作为深色主按钮，新增 GitHub 协作按钮作为空心次按钮。新按钮直接复用远端现有 `.button` 的尺寸、字号、字重、圆角、动效和可访问性状态，仅使用现有 coral 色区分边框与文字，不新增组件体系。
...
1. 中英文页面首屏都显示同尺寸的下载按钮和 GitHub 协作按钮，下载为主按钮，GitHub 为空心 coral 次按钮。
