...
export function argsFor(c, models = []){
 ...
 if(c.kind.startsWith('model')) {
  const model = models.find(entry => entry.archetypeId === 'gpt-image-2');
  const mode = model?.modes.find(entry => entry.modeId === model.defaultModeId);
  const tier = c.kind === 'model2' ? '2K' : '1K';
  const parameter = mode?.params.find(entry => entry.options?.some(option => option.value === tier));
  if (!model || !mode || !parameter) throw new Error(`Projected catalog does not support GPT Image 2 ${tier}`);
  Object.assign(next.shots[0], { modelKey: model.modelKey, modelVendor: model.vendor, modeId: mode.modeId, params: { [parameter.key]: tier } });
 }
 return next;
}
