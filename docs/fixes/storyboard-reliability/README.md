...
案例在 `cases.mjs` 冻结；其中 `argsFor` 只供零费用供应商回放。真实 DeepSeek 不使用它，用户原话经 UI 输入。前两轮 `params.size` 不符合 canonical resolution 的失败证据保持不变。第3轮起 argsFor 必须接收真实投影的目录，从档案声明导出参数键/值，缺前提直接失败。
...
全部资料来自合成故事与独立 profile。没有复制真实用户项目。第3轮起按用户明确授权，prepareIsolation只复制真实catalog设置到独立profile，凭证不输出、不进入证据。解压后的证据也按 DEEPSEEK_API_KEY 原值扫描，未发现凭证。
