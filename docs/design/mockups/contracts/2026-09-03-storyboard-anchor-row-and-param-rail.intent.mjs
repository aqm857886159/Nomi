...
export default {
  ...
  structure: [
    ...
    {
      name: '锚行与镜头行共用同一个容器视觉家族，不是另起炉灶的旧卡片布局',
      sameClass: ['[data-storyboard-row]', '[data-storyboard-anchor-row]'],
      // FIX4（2026-09-03）修正：锚行不再被逼进镜头行的 14/84/136/1fr 四栏栅格——那正是上一轮
      // 把 108×144 的真实锚卡压扁成 76×132 的根因。两者仍共用 shot-row 类换来的 border/radius/
      // accent 描边（视觉同族、一眼认得出「这也是一行」），但锚行内部改用 StoryboardAnchorCard.tsx
      // 自己的 flex 布局，画面格/名字行逐字照抄真实组件，不是再造一份栅格骨架。
    },
    {
      name: '参数下沿复用统一参数条',
  ...
  modeProof: [
    ...
    { key: 'seedance-2:omni', zone: 'slots', tiles: 1, array: true, contains: ['角色参考', '参考视频', '参考音频'] },
    { key: 'happyhorse:i2v', zone: 'slots', tiles: 1, array: false, contains: ['按数组传入'] },
    { key: 'happyhorse:edit', zone: 'slots', tiles: 2, array: true, contains: ['源视频', '参考图'] },
