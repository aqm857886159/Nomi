...
| **B8** | **MCP 授权确认重复/入口混淆**：连接、项目选择、生成审批可能被用户体验成三次授权；客户端 elicitation 与 Nomi GUI 可能连续确认；`session/open` 的 opaque handle 又没有自然 bootstrap | 🔴 P0/P3 前置 | ⬜ **方案已确认：客户端优先一次组合确认，Nomi 同一 challenge 兜底；见 2026-08-23 设计/审计** |
...
| C5 | **连接 / 授权 / 失败下一步文案分层**：机器需要完整 typed error，用户只需要“在客户端确认 / 在 Nomi 确认 / 重新选择项目 / 等待对账”四个动作，避免把 lease/receipt/hash 暴露到主路径 | ⬜（随 B8 同批收口）|
