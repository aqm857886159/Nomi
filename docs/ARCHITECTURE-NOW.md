...
| **对话式模型接入** | MCP 面是 4 个工具：`nomi_list_models`（读）、`nomi_await_setup`（等）、`nomi_model_setup`（6 个可撤销 action：connect_provider / choose_models / draft_adapter / check_connection / show_models / cancel）、`nomi_remove_provider`（唯一不可逆，带不透明 `ifUnchanged`）。一个工具 = 一种后果（状态 × 效果类别）；版本号与幂等键不在模型入参里，宿主按 (setupId, action, 入参摘要) 自己派生。密钥只在 Nomi 可信页面保存，模型面永远拿不到 key 的值（填 key 是返回值 `nextAction.kind=user_sees_key_page`，不是动词）。`check_connection` 是**免费**自检，失败不下架、只把模型标「未试跑」 | `electron/capabilityCore/modelOnboarding/declarations.ts:1`、`electron/capabilityCore/modelOnboarding/dispatch.ts:1`、`electron/integrationCertification/integrationSession.ts:1239`、`src/ui/onboarding/IntegrationSelfCheckPanel.tsx:21` | ❌「Agent 能直接拿 API key、自己确认花费或写一行 seed 就算接入」——三者都不成立；❌「自检通过 = 接好了」——自检只证地址与 key，`unverified` 里始终留着 `model_produces_output` |
...
| **内部 Agent vs MCP** | **两套入口合同 + 一份共用的模型可见工具注册表 + 一套领域实现**。`electron/shared/agentCapabilities/modelFacingToolRegistry.ts:92` 的 `modelFacingToolSpecs()` 是内部（lane）与外部（MCP）共用的唯一注册表，按 `spec.profiles` 声明投影出两边各自的工具面；两套入口各自的 schema/权限链最终仍调同一个领域函数（`applyCanvasToolCall.ts:674` 与 `capabilityApplyHandler.ts:716` 都调 `arrangeStoryboardToTimeline`，定义于 `sendStoryboardToTimeline.ts:84`）。仍存在的例外（未收编进统一注册表）：`electron/agentLane/laneModelRead.mts:5` 另手写一份 `nomi_read`；`electron/capabilityCore/mcpGenerationToolCatalog.ts:59-134` 手写五个生成工具 | `electron/shared/agentCapabilities/modelFacingToolRegistry.ts:92`、`electron/agentLane/laneModelRead.mts:5`、`electron/capabilityCore/mcpGenerationToolCatalog.ts:59-134` | ❌「没有共享的工具定义层」——已不成立，`modelFacingToolRegistry.ts` 是 2026-09 起的唯一 owner；⚠️ 仍有两处已知例外（`laneModelRead.mts` 的第二份 `nomi_read`、`mcpGenerationToolCatalog.ts` 手写的五个生成工具）没收编 |
