...
> 最后核对：2026-09-26 · 核对基线：`origin/main@1f39ea3cf`
...
| **Agent 运行时 · 现役 = pi lane** | #646（2026-09-09 合）已原子切换：`electron/agentLane/` 是**唯一**用户能走到的 agent 通路，旧宿主 `electron/projectAgentHost/` 已整体删除（仓库里已搜不到）。生产 IPC 在 `electron/main.ts:403` 注册 `desktopLaneIpc = registerAgentLaneIpc(createDesktopLaneDependencies(...))`；面板经 `src/workbench/ai/v4/useAgentPanelV4Data.ts:69` 的 `React.useSyncExternalStore(laneClient.subscribe, laneClient.workspace, laneClient.workspace)` 订阅 `laneViewModel` | `electron/main.ts:403`、`src/workbench/ai/v4/useAgentPanelV4Data.ts:69`、`electron/agentLane/laneHost.mts`、`src/workbench/ai/lane/laneViewModel.ts` | ❌「还在阶段 1 影子期 / 新旧两条通路并跑」——`electron/projectAgentHost/` 已不存在；❌「用户仍走旧宿主 + `agentPanelV4Projection.sortedItems()`」——旧通路随 #646 一起删除，pi lane 是唯一通路 |
| **经验沉淀闭环** | ⚠️ 随 #646（pi lane 原子切换）退役，待重接。原调用入口 `electron/projectAgentHost/projectAgentTurnExecution.ts` 与写手 `electron/experience/projectAgentExperience.ts` 已随旧宿主一起删除；`experienceExtractor.ts`/`experiencePolicy.ts`/`experienceRepository.ts` 三个文件还在，但已零生产调用者（只剩各自的单测），是孤儿代码，构不成闭环 | `electron/experience/experienceExtractor.ts`、`electron/experience/experiencePolicy.ts`、`electron/experience/experienceRepository.ts`（均零生产引用，仅自测） | ❌「回答过一次就会自动写进 Skill/模型」——原判断依然成立；❌「这条闭环还在跑」——生产侧目前没有任何代码调用这三个文件，连显式 envelope 的入口都没了 |
| **Agent 工具组怎么选** | 不按 capability 也不按 skillKey：内部 profile = `VERB_DECLARATIONS` 的一次投影（常驻 10 + 按 `internalGroup` 延迟披露的组，模型用 `nomi_request_tools` 切组）。**模型看得见什么只有一个 owner**：`verbDeclarations.ts`（名字/五槽描述/schema/示例/效果/nextAction 全从声明派生；对外 MCP 的描述与注解同源） | `electron/shared/agentCapabilities/verbDeclarations.ts`、`electron/agentLane/laneToolCatalog.ts`、`scripts/check-tool-face.ts` | ❌「按 capability 选工具组」（`agentChatPolicy.ts`，2026-09-11 已删）；❌ 契约上的 `projections.*.description`、harness 清单 `intent`、`*ModelTools.ts`（三扇门已合一） |
| **Agent 工具作用域守卫** | 审批闸按契约 `effect/effectClass` 判（动词 `effect` 与之对账，装配期 A1 抛）；付费边界名字只住 `method` surface，内部 profile 一个都进不来 | `electron/shared/agentCapabilities/verbDeclaration.ts`、`paidBoundary.ts` | ❌ 以为只要 descriptor 存在就能调用 |
...
| **两条分镜产出路径** | 渲染端 planner（`storyboardPlan.ts:180-182`）**声明** subtitle/dialogue/transition；Agent 工具 `propose_storyboard_plan`（旧通路走 `canvasWrite.ts:183` 的 `z.record(z.unknown())`）**不声明**。→ 走 Agent 出的分镜没有字幕/转场 | `src/workbench/generationCanvas/agent/storyboardPlan.ts:180-182`、`electron/shared/agentCapabilities/canvasWrite.ts:182-183`（typed 那份在 `canvasModelShapes.ts:108-127`，新通路 `laneCanvasTools.ts` 已 extend，旧通路还没有） | ⚠️ 这是**真实缺口**，不是设计意图 |
...
| **对话式模型接入** | MCP 只提交公开连接资料并驱动 `begin -> credentials -> propose -> start`；密钥只在 Nomi 可信页面保存。**接入全程不花钱**（2026-09-12 拍板）：认证是一次免费自检（鉴权 + 拉模型清单 + 说明卡形状），所以没有 confirm 这一跳、没有签名 challenge、没有 opaque receipt。propose 之后会话停在 `ready_to_certify`，**谁拥有会话谁就能调 start**——外部宿主不必等任何人在 Nomi 里点。自检通过后模型才发布进普通目录，并如实标「未试跑」 | `electron/capabilityCore/mcpIntegrationTools.ts:4`、`electron/shared/integrationContract.ts:10`、`electron/integrationCertification/integrationSession.ts`（`start`）、`src/ui/onboarding/IntegrationConfirmationPanel.tsx:21` | ❌「Agent 能直接拿 API key」——不成立，密钥只在 Nomi 页面里存。❌「接模型要先确认花费」——2026-09-12 起不成立：那一关（`needs_spend_confirmation` / `awaiting_human_confirmation` / `human_confirmed` 三档 + 挑战 + 收据）已整个删除，它唯一的出口写死 `ownerClientId === "nomi"`，把外部宿主永久卡在半路（2026-09-12 真实验收 §P0-1）|
...
| **内部 Agent vs MCP** | **两套入口合同 + 一份共用的模型可见工具注册表 + 一套领域实现**。`electron/shared/agentCapabilities/modelFacingToolRegistry.ts:92` 的 `modelFacingToolSpecs()` 是内部（lane）与外部（MCP）共用的唯一注册表，按 `spec.profiles` 声明投影出两边各自的工具面；两套入口各自的 schema/权限链最终仍调同一个领域函数（`applyCanvasToolCall.ts:674` 与 `capabilityApplyHandler.ts:716` 都调 `arrangeStoryboardToTimeline`，定义于 `sendStoryboardToTimeline.ts:84`）。`electron/agentLane/laneModelRead.mts` 已改为消费 `modelCatalogReadSpec()`；仍存在的例外是 `electron/capabilityCore/mcpGenerationToolCatalog.ts:59-134` 手写五个外部生成工具，Phase 0 将其作为待评估的第二套声明面冻结，禁止继续扩充 | `electron/shared/agentCapabilities/modelFacingToolRegistry.ts:92`、`electron/agentLane/laneModelRead.mts:5`、`electron/capabilityCore/mcpGenerationToolCatalog.ts:59-134` | ❌「没有共享的工具定义层」——已不成立，`modelFacingToolRegistry.ts` 是 2026-09 起的唯一共享注册表；⚠️ 外部五个生成工具仍未收编，未完成前不能宣称工具面完全收口 |
