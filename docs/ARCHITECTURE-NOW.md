...
| **Agent 运行时 · 阶段 1 影子期** | `electron/agentLane/` 是一条**跑得通但用户走不到**的第二条通路：pi `AgentHarness` + `JsonlSessionRepo`（落 `<project>/.nomi/agent-sessions/`）→ 有序 `LanePart[]`（每段带 `sequence`）→ `laneViewModel` → **现有 v4 组件**。今天只接了 `document.read` / `document.write` 两个能力。它**不注册 IPC、preload 不暴露、bridge 无字段**，所以用户仍然只有旧通路一条路（P1）；CI 每轮用同一份 loopback 剧本逐项比对新旧两条通路的文字/工具/顺序/花费，不一致就红 | `electron/agentLane/laneHost.mts`、`src/workbench/ai/lane/laneViewModel.ts`、`electron/agentLane/laneShadowStructure.test.ts`（不可达断言）、`tests/agent-runtime/lane-shadow-parity.test.mts`（影子比对）、`docs/plan/2026-09-07-agent-runtime-rebuild.md` §11 | ❌「新旧两条通路 = 并行版」——影子期用户走不到新的那条，判据是机器断言不是承诺；❌「顺序还是渲染层排出来的」——新通路里 `sequence` 由主进程按 pi 转录走序赋值，`laneViewModel` **拒绝**一份乱序投影而不是替它排好；⚠️ 旧通路（`projectAgentHost/` + `agentPanelV4Projection.sortedItems()`）**仍然是用户看到的那条**，阶段 4 才切 |
