...
| **「这个模型现在能不能用」** | 全 App **唯一**一份判据：供应商在且启用 → 模型启用 → 目录发布资格成立 → 这家钥匙此刻解得开；不可用带一个封闭枚举的 reason。主进程在唯一的目录投影出口随每行模型下发 `availability`，渲染层（设置页计数 / 首页横幅 / 助手下拉 / 画布与分镜选择器 / Agent 面板默认行）全部读它、不重算，缺字段 fail-closed。角色（text/image/video、能不能发工具调用）是压在它之上的过滤器 | `electron/shared/modelAvailability.ts:81`、`electron/catalog/catalogModelAvailability.ts:25`、`electron/catalog/catalogStore.ts:283`、`src/config/modelCatalogCache.ts:113`、`scripts/check-model-availability.mjs` | ❌「各个界面自己拼 `vendor.enabled && hasApiKey && published` 就行」——2026-09-12 真实验收 P0-10 里五份副本当场给出两个答案；⚠️「这一家连上了没」是**另一个**问题（接入抽屉分组、自动适配、素材上传宿主），允许单独判，理由逐条登记在门岗里 |
