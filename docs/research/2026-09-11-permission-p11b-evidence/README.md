...
这两份报告跑出来的时候，走查里那两处等待还是写死的墙钟数（回合 180s/60s、等卡 20s）。
`check:test-waits` 不许这么写（R18），已改成从站点预算派生
（`stationTimeout({ turns: 1 })` / `stationTimeout({ operations: 4 })` / `stationTimeout({ operations: 2 })`）。
三处**都只变长**，所以报告里的两个率是下界：等得更久只可能把「没等到」变成「等到了」，
反过来不可能。复跑时数字若更高，先看是不是这条差异，别急着当成模型变好了。
