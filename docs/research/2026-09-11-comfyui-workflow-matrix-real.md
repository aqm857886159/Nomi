...
两个是**各自独立**的结构性死路（不是配置问题、不是可发现性问题），复现率 3/3；
对照实验证明：即便把「启用」那道闸绕过去，导入照样存不下来。
仓库里唯一覆盖这条路的真机走查今天在第一次点击就死了（BUG-11），这就是它们能活到今天的原因。
...
### BUG-11（覆盖缺口）仓库里唯一一条真机 ComfyUI 走查，今天在第一次点击就死了

- **现象**：`scripts/comfyui-utility-workflow-walkthrough.mjs`（`fb7df047c` 加入，在本次 base 里）
  声称证明了「② 导入成功，落进目录 … ④ 一个字都不打直接生成，真跑出图」。
  本次**照原样跑了一遍**：30s 后超时死在**第一句** `getByRole('button', { name: '接入模型' })`——
  那个按钮现在叫**「连接模型」**。它还找 `'本机 ComfyUI'`（现在是「本地 ComfyUI」）、
  `'分析工作流'`（现在是「分析」）。也就是说：**自这些文案改名以来它一次都没跑过**，
  而它的结论还挂在那儿当证据。
- **为什么没人发现**：它住在 `scripts/` 不在 CI 里；`check:walkthroughs` 是**静态**检查、不执行走查
  （docs/lessons 里「gates 绿 ≠ 走查跑过」正是这条）。
- **更要紧的**：它一上来就 `writeFileSync(model-catalog.json, { vendors: [{ enabled: true }] })`，
  **从一个真实用户到不了的状态起跑**——恰好把 BUG-1 跳过去了。
  唯一覆盖这条路的测试，把这条路上两个坏掉的闸中的一个直接写成了前提。
- **证据**：`/tmp/comfy-utility-walk.log`（本次实跑）
- **归到**：覆盖缺口（不属五步，但决定了前面 3 条阻断为什么能活到今天）· **已知？** 否。

### 对照实验：BUG-2 不是 BUG-1 的附带后果，是独立的第二道坏闸

把 vendor 预置成 `enabled: true`（= 上面那条旧走查的起跑状态，真实用户到不了），再走界面导入：

| 观察 | 结果 |
|---|---|
| ComfyUI 卡片是否进入「已连接」分支（预置模板 / 工作流列表露出来） | ✅ 是 |
| 点「导入」后是否仍弹「确认接入并开始验证」 | ✅ 仍弹 |
| **工作流是否落盘** | ❌ **仍然不落** |

**结论**：就算有人替你把 BUG-1 绕过去，导入照样存不下来。两条是**各自独立**的阻断，
得分别修；也说明旧走查那句「导入成功，落进目录」**今天从它自己的起跑点也跑不出来**。
（这是诊断对照，不是验收走查——预置状态只用来定位哪道闸坏，不当作「能用」的证据。）
...
11. **修 `scripts/comfyui-utility-workflow-walkthrough.mjs` 并把它挪进 CI 能跑到的地方**（BUG-11）：
    它现在死在第一次点击、且从 `enabled:true` 预置态起跑。选择器修好只是第一步，
    更要紧的是**让它从真实用户的起点走**——否则修好了也只是继续替我们跳过 BUG-1。
...
- 走查不许把 `model-catalog.json` 的 `enabled: true` 写成前提：接入类走查必须从
  「什么都没配」起跑，否则它测的是一个用户到不了的世界（本次 BUG-11 就是这么漏的）。
