...
checkedAt: `2026-09-02`（Asia/Shanghai）。官方原文入口：<https://docs.kie.ai/llms.txt>；本次按要求抓取 `.md` 页面。代码基线为本分支 rebase 后的 `a9112cda`。表中“wire”是渲染后请求的字段名；`MATCH*` 表示字段/模式与文档一致，但 KIE 的结果/回链/有效期存在下方双源冲突。
...
| `seed-kie-kling-3-text_to_video` | t2v | <https://docs.kie.ai/market/kling/v3-omni-text-to-video.md> | `prompt,image_urls,mode,duration,aspect_ratio,sound,multi_shots` | MATCH* |
| `seed-kie-kling-3-image_to_video` | i2v | <https://docs.kie.ai/market/kling/v3-omni-image-to-video.md> | `prompt,image_urls,mode,duration,aspect_ratio,sound,multi_shots` | MATCH* |
...
模式投影已覆盖 i2v/first-last/omni：帧 URL 与 `reference_*_urls` 不并发；非当前模式空键在模板渲染时丢弃；Kling 3 的 `multi_shots` 已存在，因而此前 BUG-4 不再是当前漂移。Seedance/Wan 官方页面要求 `last_frame` 依赖 `first_frame`，参考素材有数量/时长上限；合同测试继续锁这些组合，不以一次 HTTP 错误推导字段含义。

## 三处 KIE 文档与实测双源冲突

| 主题 | 官方页面文字 | 线上/代码证据 | 处理 |
|---|---|---|---|
| 响应字段类型 | 多个页面示例直接展示 `resultJson` 内的 `resultUrls` | 已接受的 KIE wire（`kieGptImage2.test.ts`、`kieWan30.test.ts`）返回 JSON 字符串；解析后再取 `resultUrls[0]` | 保留现有 parse + path；不按文档示例改成对象直读 |
...
关键原文摘录（短引）：Kling 页面列出 `multi_shots`；Seedance/Wan 页面将参考字段列为 optional 并用互斥约束区分帧与 reference；Suno 页面要求通过 task detail 查询结果。以上只作为文档依据，解析冲突按线上 wire 优先并标注为 `DOC↔LIVE`。
