...
- **结果**：节点目录、图片模型列表、Agent 图片/视频模型列表、Skill 面板、手动/自动生成、发布字段、历史对话、新对话不可分享、缩放/小地图/连线/吸附和 AutoLink/参考/标记/风格状态均已实测并保存截图/DOM 文本；视频 135 积分在 20 积分余额下的购买拦截也已实测，状态 `checked`。导演台实际打开后的模式变化、无效上传错误、音频/视频成功产物仍 `unverified`。
- **证据**：`33-all-feature-menu-actions.*`、`38-publish-form.png`、`39-storyboard.*`、`40–57` 号旅程证据；稳定证据根目录的 `libtv-state-matrix.json` 已加入 S12–S15。
