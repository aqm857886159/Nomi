...
## 复验（2026-09-11，两条红修完后重跑）

修法与根因见 `docs/fixes/2026-09-11-pr720-walkthrough-reds.root-cause.json` 与
`docs/plan/2026-09-10-ux-feedback-triage.md` 的「B6」一节。同一台机器、同样的隔离 profile、
同样的三条脚本重跑（截图已覆盖在 `tests/ux/shots/pr720-walkthrough/`）：

| 脚本 | 结果 | 关键数字 |
|---|---|---|
| `pr720-ux-geometry.walk.mjs` | ✅ **10 条全绿** | #5 四条真人路径（最顶项/最底项 × 慢 1px·30ms / 快 8px·4ms）**全程不关、目标项全部可点**（按钮 x[83,115]y[591,623]、菜单 x[123,271]y[458,623]，菜单仍高过按钮 133px）；#7b 高度 86 → 打满 158 → 清空 **86** → 失焦 **86** → 再打满 158 → 再清空 **86**，清空时 `scrollHeight=44`（= 一行 + padding，不再是被撑开后的 116） |
| `pr720-prompt-language.walk.mjs` | ✅ 2 条全绿 | 中文界面提示词 233 字 / 最长英文串 0；English 界面回复英文（最长英文串 29）而提示词仍中文 208 字 |
| `pr720-apimart-key.walk.mjs` | ✅ 2 条全绿 | 对话行 0→5 项、图片/视频 52→52 / 54→54（106 项一个不少）；成功卡文案已改成如实的「APIMart 已接入，预置模型可用 · 33 个预置模型现在就能在画布和 Agent 的模型列表里选到」 |

上面「两条红的现象与根因」两节保留原样，作为修前的现场记录，不回改。

另外两条观察也一并处理了：**成功卡文案**已随真实发布状态 derive（新增 `curatedModelsPublished`
一态）；**界面语言中途切换不改回复语言**经查**不是设计如此**——`openLane` 的 `systemPrompt` 是
string 快照，`buildLanguageRule()` 在开 workspace 那一刻只求值一次，而 `laneHost` 每回合重拼时
复用的是那个闭包常量。契约已放宽成 `string | (() => string)`，每回合重新求值，已开着的 lane
下一个回合就改口。（仍然保留的行为：**正在跑的那一个回合不会中途改口、历史消息不翻译**。）
