...
| 文件 | 一句话 | 状态 |
|---|---|---|
| [2026-09-06-depth-video-canvas-node.md](2026-09-06-depth-video-canvas-node.md) | **深度视频处理节点实施计划（复刻 depth.cards，Stage1+2）**：本地 WebGPU（Depth Anything V2 + MediaPipe Pose）、三模式（深度/深度+骨架/原+骨架）、pose JSON、时间裁剪、**闭环=产物可被 video_ref 模型（Seedance）消费**；spike 已验证 | ⏳ |
