...
| [2026-09-06-agent-panel-v4-real-use-fixes.md](2026-09-06-agent-panel-v4-real-use-fixes.md) | **Agent 面板 v4 打包版真实使用一批修复**（PR #558）：收据两栏落真实入参/真实回执、同名连调折行 + 失败原因说人话、工具参数容忍模型二次序列化的 JSON 文本（唯一 owner，删两份私有拷贝）、上下文窗口一手文档表、模型弹层每类一行、分镜「不吃参考的是模式」；含**两处过渡补丁**（阶段 2 工具契约重做时删） | ✅ |
