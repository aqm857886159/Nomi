...
- [2026-09-15 MCP 接模型验收：题库进 CI + 外部宿主走通用中转路径](2026-09-15-mcp-onboarding-acceptance.md) — 30 句做成会红的门岗 `check:onboarding-usecases`（96.83% / 盲臂 36.51%）；Claude Code CLI 挂 Nomi MCP 真实闭环接 APIMart 最新图/视频模型；缺陷分「我们的」与「供应商的」两堆（🚧 进行中）
