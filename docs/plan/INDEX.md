...
- [2026-09-17 「先查别人」门岗升级：数出处 → 查覆盖面（分层表）](2026-09-17-prior-art-layer-table-gate.md) — 剪辑方案带 8 条出处过门岗却漏掉 OpenTimelineIO / MLT 整层；判据从「≥3 条出处」升到「六层分层表或封闭枚举豁免」，豁免由 PR diff（新增模块行数 / 新增运行时依赖）反查；信号选型附 167 份方案实测；顺带修掉节抽取不跳过代码块的洞；根因合同 [`2026-09-17-prior-art-layer-coverage.root-cause.json`](../fixes/2026-09-17-prior-art-layer-coverage.root-cause.json)（📋 方案待拍板）
