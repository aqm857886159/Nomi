...
- **生态里已有（规范 · 条件必填怎么写）** 同一页 §Data Types 规定 `inputSchema` 就是标准 JSON Schema；条件必填的标准写法是 `allOf` + `if/then`（JSON Schema draft 2020-12 §10.2.2：<https://json-schema.org/draft/2020-12/json-schema-core#name-if>）。所以「按 action 分支的 required」有标准表达，不需要我们自造一个 `requiredByAction` 私有关键字（R31）。
...
| 4 | schema `required: ['action']` 撒谎，三个回合各撞一次，22 次失败里 9 次是它 | 广告出去的必填集与实现的必填集是两份真相 | `mcpIntegrationTools.ts` 建 `INTEGRATION_REQUIRED_BY_ACTION` 单一表，JSON Schema 的 `allOf/if-then` 与运行时的一次性缺字段聚合校验**都从它派生** |
...
1. `pnpm run gates` 全绿（含 `check:mcp-payload` 棘轮、`check:vocabularies` 词表 owner、`check:root-cause-contracts`、`check:standard-formats`）。
