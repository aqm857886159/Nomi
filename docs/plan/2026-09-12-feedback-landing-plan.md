...
| **第 0 波新建的四条分支** | `fix/sandbox-runtime-packaging-20260912` — 打包缺口：沙箱运行时二进制没进 `asarUnpack`，所以装机版根本没有沙箱（见 §0.5.3）| 未建 |
...
### 0.5.3 沙箱这件事的三个事实（先对齐，再谈修法）

上一轮对沙箱的判断有偏差，这里把实核到的三条摆出来，B12 与 D1 都按这三条改：
...
3. **打包缺口 = 沙箱运行时二进制没进 `asarUnpack`。** 这是 `fix/sandbox-runtime-packaging-20260912` 的全部内容；不修它，0.22 装机版的沙箱一样是「没有」。
4. 附带一条：**`sandboxInactiveReason` 没有任何 UI 消费者**——主进程算出了「为什么没沙箱」，但这个值在界面上零处被读。这正是 B12「对人可见」那半的实锤。
...
| ⚠️ **前提已更正（§0.5.3）** | ① 09-11 那次会话在 **dev** 下跑、沙箱**是开着的**，卡片显示「越界」是**设计如此**不是 bug；② 用户装的 **0.21.0 早于沙箱提交 `ec30e0150`**，装机版里**根本没有沙箱**；③ 打包缺口 = **沙箱运行时二进制没进 `asarUnpack`**，归第 0 波的 `fix/sandbox-runtime-packaging-20260912`，**本批不重做**；④ `sandboxInactiveReason` **零 UI 消费者**——主进程已经算出「为什么没沙箱」，界面一处都没读，这就是本批「对人可见」那半的实锤。 |
...
| 依赖 | B3（同文件 `codingCommandPolicy.ts` / `laneCodingSandbox.mts`）；**`fix/sandbox-runtime-packaging-20260912`**（打包缺口不修，装机版上这批没有可显示的「沙箱开着」状态可验）|
| 规模 / 执行者 | S+M / Opus |
...
⚠️ **先更正一个前提（§0.5.3）**：「沙箱不可用的机器」不是边角情况——**用户手上的 0.21.0 早于沙箱提交 `ec30e0150`，装机版里根本没有沙箱**；`fix/sandbox-runtime-packaging-20260912`（运行时二进制进 `asarUnpack`）落地之前，**「沙箱不在」就是装机版的常态**。所以 D1 不是「少数机器怎么办」，是「0.22 发出去之后默认怎么办」——这抬高了这一问的分量，但**不改变下面的建议**。
...
**§0.5.3 的三条事实让这道题更清楚了**：用户装的 0.21.0 **根本没有沙箱**（早于沙箱提交 `ec30e0150`），而 0.22 装机版在 `fix/sandbox-runtime-packaging-20260912` 修好之前**也没有**。也就是说「沙箱不可用」不是罕见边角，而是**眼下所有装机版用户的默认状态**——这道题的答案决定的是大多数人的体验，不是少数机器的兜底。
...
| 10 | 第 0 波 / RC 切线 | **新增** | — | 原文只有「按根因族分批」一条线，没有「这周发 0.22」那条线。按用户 09-12 拍板补 **§0.5**：切线尺子、装什么 / 不装什么、沙箱三事实、节奏图 |
...
**③ 沙箱前提更正**，三条已折进 §0.5.3、B12 与 §6 的 D1：09-11 那次会话跑在 dev 且沙箱**是开着的**（「越界」卡片是设计如此）；用户装的 0.21.0 **早于沙箱提交 `ec30e0150`**，装机版**根本没有沙箱**；打包缺口 = 运行时二进制没进 `asarUnpack`；`sandboxInactiveReason` **零 UI 消费者**。第三条把 D1 从「少数机器怎么办」抬成「0.22 发出去之后的默认状态是什么」——**但不改变建议 A′ 本身**。
