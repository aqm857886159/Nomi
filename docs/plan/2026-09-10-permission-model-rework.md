...
## 八、P1 的实施分期（正本在此，细节不在此复制）

P1 这一期按「先接进面板，再把改参数之后的行为修对」拆成两段，两段的**范围 / 不动项 / 回滚 / 验收门**
都写在实施文档里，本文件只留指针——同一件事写两遍，迟早会有一遍是旧的（P1 加新必删旧）。

| 段 | 做了什么 | 正本 |
|---|---|---|
| P1.1a | 付费确认卡接进 Agent 面板介入槽（宿主投影 → 介入槽 → 卡体那张生成框）；倒计时删除；confirm→production 夹具 | [`docs/plan/2026-09-11-permission-p1-implementation.md`](2026-09-11-permission-p1-implementation.md) |
| P1.1b | **卡上改完参数之后的那四件事**：即时重报价（同一条算式搬进契约层，渲染层本地重算）、回写只发生在按下「生成」那一刻、全部/逐镜两层覆写、R30 两组数字 | 同上，[§ P1.1b](2026-09-11-permission-p1-implementation.md#p11b--改参数之后的那四件事2026-09-11-用户拍板本轮实施) |

P1.1b 的三条已知缺口（`×N` 在面板宿主里改不动东西 / 生产提交段这条走查跑不到底 / 正式报价与本地
估算不一致时只原地换数不出声）同样记在实施文档的「已知缺口」里，不在这里另记一份。
