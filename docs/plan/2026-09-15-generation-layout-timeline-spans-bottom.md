...
## 第二轮（09-15）：同一句反馈的另一半 —— 收起后的闭环

第一轮交出去时登记了一条 `residual_risk`：**Nomi 面板收起后那条浮起的输入条压住底部带；时间轴也收起后它还压住画布底部那颗「时间轴 N 段」胶囊，于是时间轴叫不回来。** 用户原话的「不小心点了下面的时间轴收不回去了，本来我记得可以收回」= 收得回去 + 叫得回来，是同一个闭环，所以这一轮补完。

两条直接原因、两个 owner：

1. **收起态的坞锚错了层**。收起时 `AssistantPane` 是 `absolute inset-0`，但它没有确定的网格落位，包含块按规范退回网格容器的 padding 边 = 整个工作区，于是坞锚在工作区下沿、落在底部带上。修法是外壳把落位写明（`row-start-1 row-end-2 col-span-full` = 内容行），包含块随之变成那一格网格区域。**两条轴都要写起止两端**：只写 `row-start-1` 时实测仍量到整个工作区（top 对、bottom 错），界面上看不出区别——这条已写进结构测试。
2. **避让名单的范围画小了，而且有两份**。`useCanvasBottomDockRects` 与 `GenerationWorkspace` 里就地写的一份，各自持有标记名与范围，两份都钉在 `.workbench-generation__canvas` 子树上；收起坞是工作区的孩子、不是画布的孩子，于是在两份名单里都不存在。修法是合成一个 owner（`src/workbench/generation/workspaceBottomDocks.ts`）并把范围提到外壳层，坞自己声明标记。

顺带处理掉这一轮自己带出来的两件事（D4：不许把一个受害者换成另一个）：

- 坞收进内容行后落在画布下沿，那儿常驻画布工具簇 → 坞的空当从「走带条特例」扩成「让开横向重叠的底部停靠区」，纯函数 `bottomDockClearanceFrom` + 单测；走查另加一条**上界**判据「别抬太高」（超过 24px 余量即红），因为「不压住」单独一句可以靠把坞顶到半空恒真。
- 让位关系必须**单向**：坞跳过让位者（`data-bottom-dock-avoids`），胶囊与选择浮条照旧把坞算进障碍。两边互相当障碍会让位置不收敛。

### 先查别人（第二轮增量）

- **规范原生**：CSS Grid 规范对「网格容器的绝对定位子元素」的包含块规则——某一轴的落位是 `auto` 时，该轴那两条边退回网格容器的 padding 边，https://drafts.csswg.org/css-grid/#abspos-items 。这条正是 `row-start-1` 单写不生效的原因，本轮的修法完全落在规范给的行为上，没有新增机制。
- **我们自己**：标记纪律与「现量不写常数」的写法照抄 `src/workbench/generationCanvas/reactFlow/useCanvasBottomDockRects.ts:11`（本轮把它的标记与范围抽成 owner，它自己改成消费者）；空当的纯函数与钳位口径照抄同文件邻居 `src/workbench/ai/v4/agentPanelV4DockClearance.ts:22`（走带条那一坑的钳位教训直接复用）。
- **结论**：仍然全部用已有——一个 owner 模块（把两份重复实现合成一份）+ 一条纯函数 + 一个声明式角色属性；没有引入布局库，没有 z-index / 偏移兜底。
