# R30 · 真实模型腿 · deepseek-chat · 2026-09-14T03:48:54.049Z

- 选对工具率（首调 ∈ expectedFirst）：**37/42**
- 入参写对率（每次调用都过 prepareArguments → pi 校验）：**42/42**
- 回合成功率（选对 ∧ 入参对 ∧ 轨迹按序 ∧ 无禁用动词 ∧ 不声称已生成）：**20/42**
- tokens：prompt 1258766（其中缓存命中 1230592）· completion 10215

## 按语言
| lang | n | 选对 | 入参 | 回合 |
|---|---|---|---|---|
| zh | 26 | 22/26 | 26/26 | 13/26 |
| en | 16 | 15/16 | 16/16 | 7/16 |

## 按新老手
| persona | n | 选对 | 入参 | 回合 |
|---|---|---|---|---|
| novice | 21 | 19/21 | 21/21 | 10/21 |
| expert | 21 | 18/21 | 21/21 | 10/21 |
...
| artifact | 1 | 0/1 | 1/1 | 0/1 |
| price | 1 | 1/1 | 1/1 | 1/1 |

## 逐句
| id | 用户说 | 首调 | 轨迹 | 选对 | 入参 | 回合 | 禁用/错误 |
|---|---|---|---|---|---|---|---|
| R01 | 生成一张开场图 | read_script | read_script → look_at_canvas → list_models → draft_shots → generate | ✗ | ✓ | ✗ |  |
| R02 | 帮我把这段文案拆成 6 个镜头 | read_script | read_script | ✓ | ✓ | ✗ |  |
| R03 | 把第 3 镜的提示词改成夜景 | look_at_canvas | look_at_canvas → draft_shots | ✓ | ✓ | ✓ |  |
| R04 | 这 6 镜全部生成 | look_at_canvas | look_at_canvas → generate | ✓ | ✓ | ✓ |  |
| R05 | 第 2 镜用 Kling 重新出一版，5 秒 | look_at_canvas | look_at_canvas → list_models | ✓ | ✓ | ✗ |  |
| R06 | 把这张图的背景换成雪山 | look_at_canvas | look_at_canvas → list_models → list_models | ✓ | ✓ | ✗ |  |
| R07 | 先别生成，我看看你排的镜头 | read_script | read_script → look_at_canvas | ✗ | ✓ | ✗ |  |
| R08 | 现在画布上有什么 | look_at_canvas | look_at_canvas | ✓ | ✓ | ✓ |  |
| R09 | 那张图生成好了吗？花了多少 | look_at_canvas | look_at_canvas | ✓ | ✓ | ✗ |  |
| R10 | 取消正在跑的那条视频 | look_at_canvas | look_at_canvas → cancel_job | ✓ | ✓ | ✓ |  |
...
| R17 | 用「口播成片」这个技能来做 | read_skill | read_skill → read_script → look_at_canvas → list_models → draft_shots | ✓ | ✓ | ✓ |  |
| R18 | 把这套做法存成技能 | read_script | read_script → look_at_canvas → read_timeline → save_skill | ✗ | ✓ | ✗ |  |
...
| R26 | 做一张分镜对照表放画布上 | read_script | read_script → look_at_canvas → make_artifact | ✗ | ✓ | ✗ |  |
| R27 | Make me a 10-second product teaser | read_script | read_script → look_at_canvas → read_timeline → list_models | ✓ | ✓ | ✗ |  |
...
| R30 | Don't generate anything yet, just lay out the shots | read_script | read_script → look_at_canvas → list_models → list_models → draft_shots | ✓ | ✓ | ✓ |  |
| R31 | Swap the model on every shot to Seedance | look_at_canvas | look_at_canvas → list_models → draft_shots | ✓ | ✓ | ✓ |  |
