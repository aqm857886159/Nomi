...
2. **代理一生效，取回侧就把策略整段跳过。** 调用点自己写了句 `if (代理生效) { 不判了 }`。
   这不是「判得宽松」，是**策略 owner 被绕过了**——下一条出站路径照抄这个 `if`，owner 就又分裂成两份。
...
测试把这条钉死了——把老逃生口（`if (route === "proxy") return allowed`）变异回去，
`electron/vendor/vendorOutboundGuard.test.ts` 与 `electron/hardenedFetch.test.ts` 立刻翻红。
