...
## 2026-09-11 作废：本文件描述的那版实现已被用户否掉

用户 2026-09-11 裁决「恢复旧设计」。上面「不依赖选中的常驻外侧热区」那条合同**不再成立**——
它正是 `canvas-card-stack` 回归的机制（常驻 112px 带子把卡片之间的连线整条吞掉）。
当前交付的语义、迁移前 ↔ 恢复后的逐项几何对照、以及红→绿证据，写在
[2026-09-09-magnetic-walk-contract.md](2026-09-09-magnetic-walk-contract.md) 的「2026-09-11 拍板：恢复旧设计」一节。

本文件保留作为**当时怎么想的**的记录，不要再当规格读。原先引用的 `performance-summary.json`
测的是被否掉的那版实现，已随实现一并删除；恢复后的把手几何与 `origin/main` 一致，不再声称任何性能增减。
