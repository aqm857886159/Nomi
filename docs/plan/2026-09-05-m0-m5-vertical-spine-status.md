...
- 2026-09-05 最新真实 APIMart run 的 M2 `capability_receipt_unresolved` 进一步定位到 hydration-to-surface owner race：项目/Storyboard 已可见时，React persistence effect 仍可能尚未安装 active save target，canonical patch 遂在内存 receipt commit 后读到空 durable owner。共享 session 新增 `waitForActiveWorkbenchProjectSaveTarget` owner signal，canonical renderer barrier 在保存前等待既有单一 owner；不新增 writer、不改 runner sleep。新增 canonical/session 红绿回归并更新 schema-v3 合同；真实 Electron 重跑待验证，M2 仍保持 blocked/partial evidence 边界。
...
- 2026-09-05 最新 APIMart follow-up：M5 重启后的 active workspace 已到 URL `step=storyboard`，`editorCount=1` 且 row 2 选中（`row2=1`）；但 M2 canonical `patch_shots` 因 `capability_receipt_unresolved` fail-closed，未形成 durable receipt/revision，因此 M2/M4 durable 与完整 M5 仍阻塞。该 run 的证据根以本机隔离 runner 收据为准，凭据不写入台账。
...
交付边界：PR #486 head 为 `215bbfa0`；当前分支有 follow-up 未提交改动，未 push 新 commit。上述 `PARTIAL_PROOF`/`BLOCKED_ENVIRONMENT`/`UI_PRODUCTION` 状态不允许升级为 `VERIFIED`，PR/worktree 盘点与后续阶段清单见本台账上文。
