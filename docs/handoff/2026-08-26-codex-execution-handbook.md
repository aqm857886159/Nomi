...
- **管道会吞掉退出码。** `pnpm run test | tail` 的 exit 是 tail 的。**永远** `cmd > /tmp/x.log 2>&1; echo exit=$?`。
  ...
  - **这条最难防，因为知道规则不管用。** 2026-08-26 03:20 实录：我在**写完这份文档之后**验证一个新门岗，随手写了
    `node scripts/check-xxx.mjs 2>&1 | tail -3; echo "exit=$?"`，屏幕上门岗明明打印了 `❌ ...`，`exit` 却是 `0`——
    因为那是 `tail` 的退出码。**报错文字和退出码同时出现在眼前、彼此矛盾，而我差点按退出码下结论。**
    不要依赖"我记得这条规则"；要养成**手指上的习惯**：验证命令里永远不出现管道，先重定向到文件，再单独 `tail` 看内容。
    判据：**只要 `echo $?` 前面有 `|`，这个退出码就是假的。**
