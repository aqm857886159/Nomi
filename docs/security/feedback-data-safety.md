...
- **pre-commit / pre-push hook**：`scripts/install-git-hooks.cjs`（`pnpm install` 的 postinstall 自动跑）安装到当前 Git hooks 配置。pre-commit 只扫 staged 敏感数据；pre-push（`scripts/ponytail-review-hook.mjs`）校验要推的树有没有被交工前的 `pnpm run review:branch` 评审过（R25，2026-09-15 起评审不在提交时刻跑）。普通 worktree 使用其 configured hooks 路径；linked worktree 只有在 `extensions.worktreeConfig=true` 时才写入专属目录，无法隔离则跳过并提示，避免一个分支改坏并行 worktree。手动补装：`node scripts/install-git-hooks.cjs`。
