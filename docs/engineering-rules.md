...
**规则**：任何用来证明某事成立的东西——单测、走查断言、门岗规则、CI 步骤、获批样张——
交付前必须**先破坏一次被验对象，确认它精确报红**。只看到绿不算验过。

**为什么**：这个项目已有 36 道 `check:*`、棘轮、fail-closed 合同，全在防「做错了」，
它们共享一个前提——**报上来的信号是真的**。而记忆里约 1/4 的坑不是做错了，是
**检查坏了但看起来是好的**。这类的危险不在红，而在「绿得跟真绿一模一样」：

| 实例 | 表面 | 实际 |
|---|---|---|
| `expectAbsent` 首次采样即过 | 绿 | 计数本就是 0，15 秒 timeout 一秒没用上 |
| 死选择器 | 假红 / **假绿** | 存在型断言必红（浪费一个月）；不存在型断言**永远通过** |
| `pnpm test \| tail` | exit 0 | 那是 `tail` 的退出码；一条测试都没跑也「全绿」 |
| `check:walkthroughs` 通过 | 像跑过走查 | 它是静态检查，压根不执行走查 |
| CI 步骤写好了 | workflow 里有 | 挂错 scope，触发条件永不满足 |
| 获批样张在仓库里 | 可以对账 | 已被走查覆盖成当前 UI，对账＝拿现状比现状 |

**怎么做**（成本很低，就是多跑一次）：
1. 写完断言，**故意破坏它验的东西**（删掉那个块、改掉那个值、把条件改回错的版本）。
2. 确认它报红，且**报红的位置和理由都对**（不是碰巧因为别的原因红）。
3. 还原，确认变绿。
4. 在 commit message 里写一行「已验会红：<怎么破坏的> → <红在哪>」。

**两个易错点（都真栽过）**：
- `toContain` / `indexOf` 是**子串**匹配：把 `data-foo` 改名成 `data-foo-RENAMED` 断言照样通过。
  破坏要用**真实回退形态**（整块删掉、挪位置），不是改个名字。
- 正则 `/g` 配 `[1]` 取到的是**第二个完整匹配**，不是捕获组。用 `matchAll` 取 `m[1]`。

**机器面**：`check:walkthroughs` 的 `mockup-overwrite`（走查不许写 `docs/design/mockups`）与
`i18n-text-as-locator`（中文 aria-label 定位走棘轮）两条规则，加规则时都按 R17 验过会红。
定位控件请用 `data-*` 测试契约（`data-settings-locale` / `data-open-settings` / `data-settings-tab-id`），
它不随 i18n 与 §1.5 控件层级调整漂移；中文文案只用来断言「用户看到了什么」，不用来导航。

## R29 fail-closed 状态必须可观测

**规则**：任何 fail-closed 守卫（拒绝写入、降级、只读、限流、熔断）除了「拒绝」这个行为本身，
必须在**共享的 health / status 契约**里有结构化表示（带足够让文案与测试 derive 的字段），
不能只以抛出的异常存在。

**为什么**：守卫拒绝得对，不等于系统表现得对。状态只以异常存在时，每个消费者都得
「先撞一次墙、catch 到、才知道」——于是**不 catch 的调用点静默退化，catch 的把主进程英文
`Error.message` 原样甩给用户**。2026-09-01 的实例：catalog 盘上版本比构建新时 `writeCatalog`
fail-closed 拒写（正确），但渲染层约一半的写入点没有 catch，用户点「启用供应商 / 存密钥 /
改地址」全无反应；走查往目录里播种也全部失败，模型选择器空着、「切换模型」点了不生效且不报错。
2026-08-31 曾按同一症状修过一次，但 prevention 落在**单个 React 组件**上，于是同类从新入口原样复发。

**怎么做**：把判定放在状态**产生处**（本例是 `deriveModelCatalogHealth`，唯一一处派生），
让它输出 `writable` + 结构化 issue + 两个版本号；产品侧与测试侧各自从这一处 derive，
不在消费端重算比较。判据自然覆盖全部现存与未来消费者，不需要逐个 call site 加 catch。

**自检**：这个守卫生效时，① 用户能不能在动手**之前**看到、并知道该做什么（本地化、含出路）？
② 自动化测试能不能在起飞点问出来、而不是靠某条断言碰巧撞上？两问都得是「能」。
