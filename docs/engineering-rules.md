...
2. Playwright 走查（R13）
3. **「同一语义有几份定义」横扫**（2026-08-27 新增固定维度，见下）
4. 落 `docs/audit/<date>-*.md`：现状 + 分级问题（带 file:line）+ 立即/中期/长期路线
5. 清掉 P0，方案级取舍留用户拍板（R3）；关键论断亲自实跑核实

### R14.1 固定维度：同一语义有几份定义（2026-08-27 加）

**为什么单列成固定维度**：2026-08-27 一天之内撞了 8 次「造了第二套」——对外 22 个 `nomi_*` 工具 vs 内嵌 17 个（6 处同事两名）、技能可见范围三套口径 / 加载机制两套 / 标识符两套、技能包格式自造信封、确认面两套、状态词表 33 套（9 套同义）。**其中一次是在写「同一语义只能有一份定义」那份文档时又编了第 4 套阶段词表。**

**根因不是纪律**：P1（加新必删旧）、R20（build-vs-buy 闸）、设计系统 §1.5（一功能一个家）早就写着，当天**一条都没触发**。因为这些规则全都假设「你知道第一套存在」——你不知道的时候，你不觉得自己在「加新」，你觉得自己在「从零建」。**这是检索失败，不是纪律失败。**

**机器能查的那一半**：`pnpm run check:vocabularies`（棘轮门岗，已进 gates 链）。它扫状态/阶段类字面量联合，新词表未登记即红，并把**已有那一套的 file:line 和差异摆出来**。baseline 分两桶：`registered`（有意独立，reason 说清为什么不能复用）/ `debt`（已知待收敛，条数只减不增）。**写不出 reason = 就是漏了**——baseline 因此会自己长成一份「系统里有哪些共享语义」的登记表，且从代码 derive，不会像手写文档那样腐烂。

**机器查不到、必须人横扫的那一半**（审计时逐条过，带 file:line 落进审计文档）：

| 维度 | 问法 | 今天的实例 |
|---|---|---|
| **工具面** | 同一个操作，内外/多入口各叫什么？行为等价吗？ | `nomi_add_nodes` vs `create_canvas_nodes` |
| **可见性/过滤口径** | 同一批东西，几个消费方各写了几套 `if`？ | 技能可见范围三套（前缀匹配 / stages 判 / 不过滤）|
| **标识符** | 同一个对象，用哪个字段当主键？两边一致吗？ | `directoryName` vs `skillKey`，且有 `findSkillRecord(key, key)` 撞运气写法 |
| **格式契约** | 我们自造了信封/协议吗？生态有标准吗？ | `.nomiskill.json` vs 标准 SKILL.md 文件夹 |
| **字段取值来源** | 同一个展示字段，取值链路只有一条吗？ | `skillIpc` 只读 manifest，漏掉 `skillStore` 已算好的 frontmatter description |
| **确认/权限面** | 有几处在做「要不要放行」的判断？ | `mcpGenerationPolicy` vs `gate.ts` |
| **规则的路径覆盖** | 同一条规则，**每条路径都落到了吗**？（列 vs 读、写 vs 改、创建 vs 删除、导入 vs 导出）| 技能可见性只过滤了 `skills.list`，`skills.read` 没过滤 —— 外部报个目录名就能取走任意一本全文 |

### R14.1.a 最难查的一族：一件事只做了一半，而做了的那一半看起来很完整

上表最后一维单独说，因为它跟前六维不同 —— 前六维是「有两份」，这一维是「**只有半份**」。

2026-08-27 一天里出现三次，形态完全一致：

| 实例 | 做了的那一半 | 漏掉的那一半 |
|---|---|---|
| 技能对外可见性 | `skills.list` 过滤了 | `skills.read` 没过滤 → **只挡住「看见」，没挡住「拿到」** |
| 技能卡片描述 | `skillStore` 算好了 manifest ∥ frontmatter | `skillIpc` 只取 manifest → 没 manifest 的一律「暂无说明」 |
| 画布操作工具 | 内嵌有运镜 / 站位 | 对外 MCP 没有 → 外部 agent 永远调不到最贵的两个能力 |

**为什么它比「有两份」更难查**：它不报错、测试也绿、代码读起来完全合理 —— 因为你看到的那一半确实写对了。今天的可见性洞如果不是多问一句「列过滤了，**读**有没有过滤」，就漏过去了。

**安全场景下它有个专属名字：防护做在展示层、没做在数据层。** 列表过滤看起来像安全措施，其实只是 UI 行为。

**审计时的问法**（比「有没有第二份」更该先问）：

> 我刚加的这条规则 / 这个字段 / 这个能力，**它的对偶路径是什么？那条路径上有它吗？**
> · 加了「列」的规则 → 「读」呢？
> · 加了「写」的校验 → 「改」呢？
> · 加了「导入」的能力 → 「导出」呢？
> · 加了内部的工具 → 外部入口呢？

**诚实边界**：上表 7 个维度里，门岗只覆盖得了「词表」这一类。格式契约、字段取值来源、名字完全不同的同义工具面，**静态扫描抓不到**——今天它们是靠真机走查（「暂无说明」那张废卡）和人反复追问「这个是不是也有类似问题」抓出来的。**没有银弹**，所以这一条必须留在人执行的审计清单里，不能指望门岗兜住。
