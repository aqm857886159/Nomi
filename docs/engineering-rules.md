...
> 这是 `CLAUDE.md` 的「按需查阅」层。`CLAUDE.md`（always 加载）= 精简核心：项目事实 + P1–P5 + D1–D6 + 规则索引 + 三闸。本文件存**触发某条规则后才查的细节**：R1–R27 详解、工作流框架、技能库映射、固化的工作纪律。
> 真相源仍单一：`CLAUDE.md` 的规则索引指明每条住哪；冲突一律以 `CLAUDE.md` 的 P1–P5 / D1–D6 为准。改触发清单同步 `scripts/claude-hooks/self-check.sh`（真相源；`.claude/` 下那份由 `pnpm install` 装），规则细节只改本文件。

# 详细规则（17 个主号 + 旧号别名）

> `CLAUDE.md` 的规则索引触发某个编号后，到这里查它的细节。
>
> **2026-09-14 合并**：L1 索引 30 条 → 17 条，**一条正文都没删**——13 个旧号的正文整段搬进了同族主号，旧号在本文件留一行别名节。引用旧号的 PR、根因合同与教训**不作废**；`pnpm run check:rule-aliases` 保证家规文件里任何 `R<数字>` 引用都解析得到。

## 编号别名表（旧号 → 现在住哪）

| 旧号 | 现在住哪 | 为什么合 |
|---|---|---|
| R6 近邻开源优先 | **R5.2** | 「先查别人」族：做方案这一档 |
| R10 CSS 只可减不可增 | **R1** | R1 的 CSS 实例（合并前就是别名行）|
| R12 巨壳门岗 | **R9** | R9 的量化门岗（合并前就是别名行）|
| R16 真实用户任务测试系统 | **R13.2** | 「完成标准」族：功能交付这一档 |
| R18 测试等待门岗 | **R17.2** | 「棘轮门岗」族 |
| R19 解决状态必须可交付 | **R11.1** | 同一条交付链的后半段 |
| R20 build-vs-buy | **R5.3** | 「先查别人」族：写通用能力这一档 |
| R23 React Flow 单内核 | **本文件 R23（不再进 L1）** | 领域实例（R1 + R8 在画布上的形态），不是工程原则 |
| R26 分层边界 | **R17.3** | 「棘轮门岗」族 |
| R28 防线建在最早能拦住的那层 | **R17** | 它就是棘轮门岗族的元规则，升为主号 |
| R29 框架四列表/逐层对照/逐字段裁决 | **R5.4** | 「先查别人」族：接框架这一档 |
| R30 Agent 真实模型数字 | **R13.3** | 「完成标准」族：Agent 这一档 |
| R31 外部格式对齐标准 | **R5.5** | 「先查别人」族：对外格式这一档 |
| R32（提案中的「真实测试」）| **R13「四件真实」** | 不新增编号，见 R13 开头的判据段 |

> 表里「现在住哪」是唯一真相源。改编号必须同时改本表、`CLAUDE.md` 的 L1 索引与 `scripts/claude-hooks/self-check.sh`，再跑 `pnpm run gen:agents`。
...
**R23 = R1 在生成画布上的实例**：生产生成画布只许 `@xyflow/react` 一个内核，禁第二 renderer / engine flag / fallback / 并行实现；迁移必须逐项等价。正文见本文件 `R23`（2026-09-14 起不再占 L1 索引一行——它是领域约束，只在碰画布时相关）。
...
**分档判据**：代价落在**我们**身上（多维护一份、更差一份）＝ R5.3 / R5.4；代价落在**用户**身上（别人的文件进不来、我们的文件出不去）＝ R5.5。R5.4 允许「有意不同」，R5.5 的「不同」必须能追到领域约束，否则就是 `non-aligned`，得绑到期日收敛掉。**偏好不是理由**——这一句五档通用。

### R5.1 查官方文档（Context7 强制）
...
### R5.3 造轮子前先过 build-vs-buy 闸（旧 R20）
...
### R5.4 接框架先出四列表 + 参考实现逐层对照 + 逐字段裁决（旧 R29）
...
### R5.5 外部格式/协议/契约必须对齐官方或事实标准（旧 R31）
...
## R6 → 见 R5.2（近邻开源优先，读真实代码）

正文整段搬进 R5「先查别人」的第二档，一字未删。
...
**两段一条链**：`R11.0` 说什么时候推，`R11.1` 说推完之后怎么称呼这件事。

### R11.0 自动 commit + push
...
## R13 完成标准：走查 · 真实任务 · Agent 数字（含旧 R16 / R30）

**这一条是 P3「全绿 ≠ 完成」的量化门。** 旧 R13 / R16 / R30 三条正文各自都写着自己是另一条的一部分（R16：「R16 是 P3 的量化门」「用 R13 的走查方法」；R30：「R30 是 P3 / R16 在 Agent 这条线上的量化形态」），执行细节早就合并在同一份 [`engineering/acceptance-walkthrough-doctrine.md`](engineering/acceptance-walkthrough-doctrine.md) 里。L2 已经合并了，L1 还分着三个号——2026-09-14 起合成一条。

### R13 的三档触发

| 触发 | 必做 | 旧号 |
|---|---|---|
| 用户可见改动 / 把任何可看的东西交给用户 | 和获批样张逐项对账 + 眼见链四问 + 位置断言 + zh/en 双语真截图 + 真人乱输路径 | R13（见 R13.1）|
| 功能交付（尤其用户可见 / 体感）| 建 ≥2-3 条「真实用户任务」跑通闭环，冒出的体验/设计/UI/UX/产品感/功能问题**全修掉**，重跑到干净 | R16（见 R13.2）|
| Agent / 工具定义 / 工具契约 / 系统提示词 / 模型档案改动 | 工具写对率 + 回合成功率两个数字写进 PR 正文 | R30（见 R13.3）|

### R13 的四件真实（缺一件，这条测试不成立）

> 2026-09-14 加。起因：第一次拿用户真实素材（4K 10-bit HEVC `.mov`、1.38 GB）跑 Nomi，当场炸两件——① 视频和图片**导入不了画布**；② S 规模（24 图 + 24 视频）**20 秒进不了画布**。而画布性能基准跑了几十轮、留了 80 多份结果 JSON，一次都没红过——因为夹具用的是 ffmpeg `testsrc2` 合成素材。详见 `docs/lessons/2026-09-14-real-media-first-run-exposed-import-and-open-time.md`。

| # | 必备件 | 什么不算 | 判据 |
|---|---|---|---|
| ① | **真实应用** | 组件夹具 / 设计实验室 / dev server 的一个页面 / `vitest` 里 mount 的一棵树 | 走查报告里有构建戳，且它与被验证的 commit 对得上（眼见链四问，R13.1）|
| ② | **真实页面输入** | 往 store 灌状态、走 E2E 桥、喂夹具 JSON 当输入、`page.evaluate` 改状态、直接写项目快照绕过导入 | 输入动作只有 `mouse` / `keyboard` / `locator.click\|fill\|hover\|dragTo` 一族；`evaluate` 只许出现在明确标注的准备阶段 |
| ③ | **真实工具轨迹** | 人手写的合法参数喂给单测 | 真模型跑出的工具调用序列 → 工具写对率 + 回合成功率（R13.3）|
| ④ | **真实素材** | `testsrc2` 色块、1px 占位图、`data:image/svg+xml`、2 秒 crf-35 的 720p | 素材登记在 `tests/ux/real-media-fixtures.json`，路径走 env `NOMI_REAL_MEDIA_DIR`；门岗 `pnpm run check:real-media-fixture` |

**取到哪件就必须是真的**：单元测试与设计实验室基线不在这条规则管辖内——它们本来就不声称证明「能用」。这条规则拦的是**拿它们当验收结论**（R13.3 的红线：外观绿不等于接好了）。

**第④件为什么必须单列**：前三件都拦不住它。走查可以完全「像人一样点」，只要点的是合成素材，就照样测不出真实解码成本，也照样走不到导入链路——夹具直接写 `project.json` 快照，导入一步都没走。**输入假了，再严谨的断言也只是在证明「假输入下没事」**（同族：`docs/lessons/vacuous-probe-passes-forever.md`）。

**缺素材时红，不是跳**：读不到 `NOMI_REAL_MEDIA_DIR` 或登记素材不存在 → 非零退出并打印缺什么。CI 素材未就位期间登记为 `docs/engineering/real-media-debt.json` 里**带到期日（≤30 天）的债**，到期即红。**不许 skip**——那正是 R17 说的「登记即放绿」，规则从第一天起就是装饰。
...
## R16 → 见 R13.2（真实用户任务测试系统）

正文整段搬进 R13「完成标准」的第二档触发，一字未删。
...
### R17.0 棘轮门岗族：三条同构的门岗共用一句元规则

旧 R17 / R18 / R26 是三个同构的棘轮门岗，三条正文里**各自抄了一遍同一句元规则**：「加规则前必须先验它会红」「基线只减不增」「被忽略的门岗等于不存在」。而旧 R28 恰恰就是这句元规则本身。所以 2026-09-14 把元规则升为主号，三个门岗降为它的实例。

**这一族的共同规矩**（新加任何棘轮门岗照办）：

1. **基线只减不增**，且**存身份不存裸数字**——裸数字放过「删一处旧的、同 commit 加一处新的」。
2. **加规则必须先验它会红**：临时塞一处违规写法 → 跑门岗确认报红且 `file:line` 点得对 → 删掉。**只验过绿的门岗不算门岗**（绿有可能是正则根本没匹配上）。有「合法例外」的规则还要补一发反向控制：把真实合法处的豁免条件临时去掉，确认它当场变红。
...
### R17.2 测试等待门岗（并行才炸的私有墙钟等待 · 旧 R18）
...
正文整段搬进 R17「防线建在最早能拦住的那层」的棘轮门岗族，一字未删。

## R19 → 见 R11.1（解决状态必须可交付）

正文整段搬进 R11「交付与状态」，一字未删。

## R20 → 见 R5.3（build-vs-buy 闸）

正文整段搬进 R5「先查别人」的第三档，一字未删。
...
## R23 React Flow 生成画布单内核与迁移等价（L2 常驻，不再进 L1 索引）

> **2026-09-14 降级**：本条三段分别是 R1（禁第二 renderer / flag / fallback / 并行实现）、P1+P4（Zustand 是唯一真相源、React Flow state 只是可丢弃投影）、R8（迁移等价清单，产品要改的另走 R8）在**画布这个领域**的实例。L1 只放「永远相关」的东西，而本条只在碰画布时相关，故降到 L2，`CLAUDE.md` 的 R1 行留半句指路。**正文一字未动，画布迁移等价审计仍以本节为规格来源。**
...
**触发**：每条要交出去的任务分支，在开 PR / 交工前跑一次。目标是让过度工程化评审发生在**还改得动的时刻**，而不是把它误当成正确性、安全或 CI 评审的替代品。

**为什么不在 commit/push 时刻**（2026-09-15 改）：那一刻活已经干完，意见出来也没人回头改——闸门只剩成本。数字与三条连带症状见 [`docs/lessons/review-at-the-moment-it-can-land.md`](../lessons/review-at-the-moment-it-can-land.md)。

### 合规流程

1. 交工前在该 worktree 跑 `pnpm run review:branch`（`scripts/ponytail-review-branch.mjs`）。范围是 `merge-base(origin/main, HEAD)..HEAD`——这条分支自己写的东西，不含主线漂移。base ref 解析不出来时明说要 `git fetch origin`，绝不静默退回整仓 diff。
...
3. **超过单次上限自动分块，不再让人拆提交**：单块上限 150 KB（`MAX_REVIEW_DIFF_BYTES`）。整段装得下就一块；装不下按**文件**切同一段范围（**不是**按提交——按提交切等于逐个评审中间态，把「17 个提交审 17 遍」请回来，而交工前要评审的是你交出去的那份最终状态，所以每个文件只出现一次）；单个文件仍超限则按 UTF-8 安全边界截断并在块首写明 `[TRUNCATED: …]`；最后把单元贴着上限装回去，调用次数逼近「总字节 / 上限」的下界。任一块报 findings，整条分支就是 findings。**二进制文件内容不进评审 diff**，改附一段 `BINARY: <added/modified/deleted> <path> (<size>)` 摘要——图片字节对精简代码评审是 100% 噪音，摘要保留「仓库变肥」信号供评审当 lean 发现提出。
...
8. **每块一个墙钟，一个常量**（`REVIEW_TIMEOUT_MS = 600_000`）。旧的派生公式与全机评审锁 `/tmp/nomi-ponytail.lock` 一并删除——超时不再拦住任何 Git 操作，两头的压力都没了（gates 那把锁不受影响）。
...
`--no-verify`、GitHub 网页/API 和未安装 hook 的环境仍可绕过本地 Git hook；若要对所有入口强制，需在 CI/分支保护中复用同一收据合同。不得把本地收据通过写成「代码正确」或「已经合入」。

### 环境与并行 worktree

- `scripts/install-git-hooks.cjs` 由 `postinstall` 调用，装 `commit-msg`（提交信息进度校验）、`pre-commit`（敏感数据扫描，**只有这一件**）、`pre-push`（收据校验）。普通 worktree 使用 configured hooks 路径；linked worktree 只有在 Git `extensions.worktreeConfig=true` 时才写入专属目录，无法隔离则跳过并警告，避免一个分支改坏并行 worktree。
...
## R26 → 见 R17.3（分层边界不许反向/循环）

正文整段搬进 R17「防线建在最早能拦住的那层」的棘轮门岗族，一字未删。

## R28 → 见 R17（防线建在最早能拦住的那层）

本条升为该族主号，正文即 R17 开头那几段，一字未删。

## R29 → 见 R5.4（接框架三份必交物）

正文整段搬进 R5「先查别人」的第四档，一字未删。

## R30 → 见 R13.3（Agent 真实模型数字）

正文整段搬进 R13「完成标准」的第三档触发，一字未删。

## R31 → 见 R5.5（外部格式对齐标准）

正文整段搬进 R5「先查别人」的第五档，一字未删。

## R32 → 见 R13「四件真实」

2026-09-14 提案里的「真实测试」**没有**拿到独立编号——完成标准已经有过三个号（R13/R16/R30），再加第四个正是这轮合并要消掉的冗余。四件真实（真实应用 / 真实页面输入 / 真实工具轨迹 / 真实素材）写在 R13 开头的判据段；唯一新增的是门岗 `check:real-media-fixture`。
