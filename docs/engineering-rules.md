...
> 这是 `CLAUDE.md` 的「按需查阅」层。`CLAUDE.md`（always 加载）= 精简核心：项目事实 + P1–P5 + D1–D5 + 规则索引 + 三闸。本文件存**触发某条规则后才查的细节**：R1–R15 详解、工作流框架、技能库映射、固化的工作纪律。
...
5. `pnpm run check:i18n` 是棘轮门岗：精确记录历史可见字面量，允许删除但禁止新增或增加次数。只有明确接受新的历史债务时才可运行 `pnpm run check:i18n:baseline`，不得用刷新基线绕过迁移。
