...
**根因**：把“工具已注册/能调用”误当成“用户能完成目标”。这会漏掉真实领域 UI 已经支持、但 Agent 提案无法表达或审批无法修改的字段，最后表现为“按钮在，能力却断了”。

任何用户可见 Agent 能力交付前，必须用同一张字段矩阵逐项对账：

| 对账层 | 必须回答的问题 |
|---|---|
| 领域 UI | 用户在真实节点/面板里能选择、输入、引用或确认哪些字段？ |
| tool schema | Agent 能否表达同一字段、类型、枚举、默认、引用和副作用？ |
| approval editor | 用户能否在副作用发生前修改最终值，而不是只能批准/拒绝？ |
| effectiveArgs | 修改后的完整参数是否随决策进入 Host，并被记录为最终提案？ |
| adapter / receipt | 同一 adapter 是否重新校验、重算 hash、执行并回执完全相同的值？ |
