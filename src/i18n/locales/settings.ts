  ...
  automation: {
    ...
    mcp: {
      ...
      customClients: {
        ...
        remove: '移除',
        copyPrompt: '复制接入提示词',
        copyPromptHint: '找不到配置路径？用提示词让 AI 工具自行找到并写入',
        copied: '已复制',
        detectedBadge: '自动检测',
        detectedHint: '该工具刚刚连上了 Nomi，但还没有配置文件路径。填入路径后可一键写入。',
      },
    },
    hosts: {
      title: '可信发起方',
  ...
  automation: {
    ...
    mcp: {
      ...
      customClients: {
        ...
        subtitle: 'Connect any MCP-capable AI tool',
        description: 'For tools without a built-in Nomi one-click, fill in their MCP config path and Nomi will sign and write the entry just like it does for Claude Code.',
        add: 'Add custom client',
        nameLabel: 'Name',
        nameHelp: 'Display name — only used in this list, does not affect the config',
        namePlaceholder: 'e.g. WorkBuddy',
        formatLabel: 'Config file format',
        formatHelp: 'MCP config format: JSON uses the mcpServers key; TOML uses [mcp_servers]. Check the tool\'s docs if unsure.',
        formatJson: 'JSON (mcpServers)',
        formatToml: 'TOML ([mcp_servers])',
        keyLabel: 'Identity key',
        keyHelp: 'Optional. A lowercase identifier (letters, numbers, hyphens) used to distinguish this client in trusted initiators. Leave blank to auto-generate.',
        keyPlaceholder: 'Leave blank to auto-generate',
        pathLabel: 'Config file absolute path',
        pathHelp: 'Where this tool keeps its MCP config. Nomi writes only this file: it backs up first, merges instead of overwriting, and uses atomic writes.',
        pathPlaceholder: '/Users/you/.tool/mcp.json',
        saveError: 'Registration failed: key conflicts with a built-in client, path is not absolute, or a required field is missing.',
        ...
        reinstall: 'Rewrite',
        remove: 'Remove',
        copyPrompt: 'Copy onboarding prompt',
        copyPromptHint: "Can't find the config path? Copy this prompt and let the AI tool locate and write it.",
        copied: 'Copied',
        detectedBadge: 'Auto-detected',
        detectedHint: 'This tool just connected to Nomi but has no config path yet. Set a path to write the Nomi entry.',
      },
    },
    hosts: {
