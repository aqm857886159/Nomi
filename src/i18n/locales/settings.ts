export const zhSettings = {
  ...
  automation: {
    ...
    videoAnalysis: {
      title: '视频拆解引擎',
      status: { connected: '本机引擎已连接', unreachable: '引擎不可达', unconfigured: '需要配置 API Token', checking: '正在检查连接' },
      refresh: '重新检查连接',
      origin: '本机引擎地址',
      token: 'e-cut API Token',
      tokenConfigured: '已安全保存；输入新 Token 可替换',
      tokenPlaceholder: '粘贴 e-cut 启动时显示的 Token',
      saveToken: '保存',
      clearToken: '清除 API Token',
      externalInference: '允许 e-cut 使用外部模型解释视频',
      externalInferenceHint: '默认关闭。关闭时只允许确定性镜头、OCR 与语音证据分析。',
      externalWarning: '开启后，e-cut 可能把完成分析所需的关键帧和文本发送到它配置的模型 API。项目文件仍由 Nomi 本地管理。',
      retention: 'e-cut 临时源视频',
      retentionDelete: '完成后删除',
      retentionKeep: '保留到手动清理',
      retentionHint: 'Nomi 项目中的原始素材不会被删除；这里只控制 e-cut 工作目录里的临时副本。',
      cleanup: '清理当前项目的临时副本',
      cleanupResult: '已检查 {{attempted}} 个任务，清理 {{removed}} 个，失败 {{failed}} 个。',
    },
  },
  general: {
    title: '通用',
...
export const enSettings = {
  ...
  automation: {
    ...
    videoAnalysis: {
      title: 'Video deconstruction engine',
      status: { connected: 'Local engine connected', unreachable: 'Engine unreachable', unconfigured: 'API token required', checking: 'Checking connection' },
      refresh: 'Check connection again',
      origin: 'Local engine address',
      token: 'e-cut API token',
      tokenConfigured: 'Stored securely; enter a new token to replace it',
      tokenPlaceholder: 'Paste the token shown when e-cut starts',
      saveToken: 'Save',
      clearToken: 'Clear API token',
      externalInference: 'Allow e-cut to use external models to interpret video',
      externalInferenceHint: 'Off by default. When off, only deterministic shot, OCR, and speech evidence analysis is allowed.',
      externalWarning: 'When enabled, e-cut may send required keyframes and text to its configured model API. Nomi still manages project files locally.',
      retention: 'Temporary source in e-cut',
      retentionDelete: 'Delete on completion',
      retentionKeep: 'Keep until manual cleanup',
      retentionHint: 'The original asset in your Nomi project is never deleted. This controls only the temporary copy in e-cut.',
      cleanup: 'Clean temporary copies for this project',
      cleanupResult: 'Checked {{attempted}} tasks: {{removed}} cleaned, {{failed}} failed.',
    },
  },
  general: {
    title: 'General',
