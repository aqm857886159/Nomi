export const zhSettings = {
  ...
  ai: {
    ...
    vendorPreference: {
      title: '同一个模型多家都有，默认走哪家',
      hint: '按这个顺序挑第一家；下面每行的供应商标签也按这个顺序排。只列已配置的供应商。',
      moveUp: '上移',
      moveDown: '下移',
    ...
    modelBox: {
      title: '模型框里显示哪些、排在哪',
      hint: '行的顺序就是模型框的顺序；隐藏的不进模型框，随时找回。每行标签按上面那张表排，蓝的是默认走的那家；你手点过的，永远听你的。',
      kindSwitch: '模型类型',
      ...
      hide: '隐藏',
      show: '显示',
      hiddenGroup: '已隐藏 · {{count}}',
      saveFailed: '没保存成功，模型框暂时还是原来的样子。再点一次试试。',
      rank: '第 {{rank}} 位',
    },
    upload: {
      title: '数据上传',
  ...
  automation: {
    ...
    mcp: {
      ...
      manage: '管理连接',
      separationHint: '连接负责“能不能连上”；每个客户端里的“允许自动发起制作”开关负责“连上后允许做什么”。',
      back: '返回自动化与权限',
      loading: '正在读取 MCP 连接…',
  ...
  general: {
    ...
    telemetry: {
      ...
      statusConfigured: '已开启；只发白名单事件和脱敏后的工具轨迹',
...
export const enSettings = {
  ...
  ai: {
    ...
    vendorPreference: {
      title: 'Same model, several providers — which one by default',
      hint: 'The first provider in this list is picked; the provider tags on each row below follow this order too. Only configured providers are listed.',
      moveUp: 'Move up',
      moveDown: 'Move down',
    ...
    modelBox: {
      title: 'Which models show in the model picker, and in what order',
      hint: 'Rows appear in the picker in this order; hidden models stay out of it and can be brought back any time. Tags follow the list above, and the blue one is the provider this row uses — one you picked by hand always wins.',
      kindSwitch: 'Model type',
      kind: { image: 'Image', video: 'Video', audio: 'Audio' },
      moveUp: 'Move up',
      moveDown: 'Move down',
      hide: 'Hide',
      show: 'Show',
      hiddenGroup: 'Hidden · {{count}}',
      saveFailed: "That wasn't saved, so the picker is unchanged for now. Try again.",
      rank: 'Position {{rank}}',
    },
    upload: {
      title: 'Data upload',
  ...
  automation: {
    ...
    mcp: {
      ...
      separationHint: 'Connections control whether an agent can reach Nomi. Each client’s “start production on its own” switch controls what it may do after connecting.',
  ...
  general: {
    ...
    screenshotOpenSettings: 'Open Settings',
    ...
    telemetry: {
      ...
      statusConfigured: 'On; allowlisted events and redacted tool traces only',
  ...
  file: {
    ...
    projectLocationSyncSteps: '3 Steps',
