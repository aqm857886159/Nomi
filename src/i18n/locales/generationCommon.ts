export const zhGenerationCommon = {
  ...
  optimizer: {
    ...
    idea: '结合用户的想法：{{idea}}',
    imageDetails:
      '把画面拆成可枚举的结构化事实并逐项补齐：主体及其关键属性（数量、外观、材质）、主体之间与环境的空间关系、动作与状态；不堆形容词、不为长而长；',
    videoDetails:
      '把内容拆成可枚举的结构化事实并逐项补齐：谁/什么（及关键属性）、在哪、什么空间关系、动作先后顺序、镜头怎么动、节奏如何；不堆形容词、不为长而长；',
  ...
  production: {
    ...
    gate: {
      ...
      openBudgetSettings: '去设置预算',
      missingBudgetTitle: '还差一个设置：硬预算上限',
      missingBudgetMessage:
        '请先到「设置 → AI 与模型 → 默认模型策略」填写制作硬预算上限（CNY）。没有明确上限，Nomi 不会提交任何付费生成。',
...
export const enGenerationCommon = {
  ...
  screenshot: {
    ...
    denied:
      'macOS has not granted Nomi Screen Recording permission, so nothing could be captured. Enable it from Settings · General.',
  ...
  canvas: {
    ...
    group: {
      ...
      connectedWithSkips:
        'Connected {{connected}}; skipped {{skipped}} — those models can’t take this kind of reference',
  ...
  optimizer: {
    ...
    imageDetails:
      'Decompose the image into enumerable structured facts and fill in each one: subjects and their key attributes (count, appearance, material), spatial relations between subjects and with the environment, actions and states. Do not pile up adjectives; do not pad for length.',
    videoDetails:
      'Decompose the video into enumerable structured facts and fill in each one: who or what (with key attributes), where, spatial relations, the order of actions, camera movement, and pacing. Do not pile up adjectives; do not pad for length.',
    outputRule:
  ...
  production: {
    ...
    description: {
      submissionUnknown:
        'The request may have reached the provider. Retrying could charge twice, so later work is paused at a safe boundary.',
      ...
      roughCutReady:
        'Open Preview to check pacing, captions, and sound. Nomi will not overwrite your edit automatically.',
      ...
      contractDeclined:
        'This decision is recorded. No generation task was submitted and no spend occurred. Revise the storyboard or scope to create a new contract.',
    },
    runTone: {
      working: 'Producing',
    ...
    gate: {
      ...
      openBudgetSettings: 'Set production budget',
      missingBudgetTitle: 'One setting is still missing: hard budget',
      missingBudgetMessage:
        'Open Settings → AI & models → Default model policy and enter a hard production budget (CNY). Nomi will not submit paid generation without an explicit ceiling.',
      directionTitle: 'Confirm creative direction',
      directionSummary:
        'Review audience, channel, tone, and truthful selling points before any model or paid API call.',
    ...
    gateScope: {
      stage: 'Current stage',
      job_set: 'This job set',
      budget_envelope: 'Spending boundary',
      export: 'Export',
      publish: 'Publish',
    },
    reconcile: {
      ...
      notFoundMessage:
        'Nomi will record “not found” and remain paused. It will not resubmit or create new spend automatically.',
    ...
    roughCut: {
      ...
      message:
        'Check pacing, captions, sound, and media in Preview first. After you accept the rough cut, Nomi will show a separate export approval.',
    ...
    contract: {
      ...
      notSet: 'Not set',
      irreversibleNote:
        'This approval covers only this production summary. Publishing, deletion, and file replacement still require separate approval.',
