export const zhOnboardingProviders = {
  ...
  drawer: {
    ...
    kindGapBody: '接入时的类型是按模型名猜的，这 {{count}} 个模型都落进了「{{kind}}」。里面若有别的类型，点它右边的类型改过来。',
    connected: '已接入',
    available: '可接入',
    network: {
      label: '网络',
  ...
  adapterVerification: {
    ...
    addedTitle: '{{count}} 个模型都已经加好了',
    addedAllPassed: '自检全部通过，画布里可以直接用了。',
    addedSomeFailed: '其中 {{failed}} 个没通过自检 —— 不影响使用，下面说了各是什么情况。',
    selfCheckMeaning: '自检是 Nomi 自己试着调一次。没通过通常是我们没猜对这家的接法，不是模型不能用。',
    failedBadge: '没通过自检',
  ...
  customCall: {
    ...
    configLabel: '② 这家还要别的密钥或参数？',
    configLabelFilled: '② 自定义配置 · {{count}} 条',
    configHint: 'Nomi 只准备了一个密钥槽。有的服务还要第二个密钥、区域、版本号——在这里自己加，脚本里用 config.名字 取出来。',
    ...
    testInputLabel: '试跑真实模式（可选）',
    testPromptPlaceholder: '试跑提示词；留空使用低成本默认样例',
    testPromptAria: '自定义调用试跑提示词',
    testParamsPlaceholder: '{\n  "first_frame_url": "https://…",\n  "reference_image_urls": ["https://…"]\n}',
    testParamsAria: '自定义调用试跑参数 JSON',
    ...
    limitNote: '两种情况这条路走不通：只支持回调通知的（你的电脑没有公网地址收不到）、只给 SDK 不给接口文档的（脚本里装不了依赖包）。',
    saveFailed: '保存失败：{{message}}',
...
export const enOnboardingProviders = {
  ...
  drawer: {
    ...
    kindGapBody: 'Types are guessed from the model name when you connect a provider, and all {{count}} of these landed in "{{kind}}". If any of them is a different type, change it with the control on its right.',
    connected: 'Connected',
    available: 'Available',
    network: {
      label: 'Network',
  ...
  modelControls: {
    ...
    retypeTitle: 'Types are guessed from the model name when connecting. Fix a wrong guess here (the call channel is rebuilt too).',
  ...
  adapterVerification: {
    ...
    addedTitle: 'All {{count}} models are added',
    addedAllPassed: 'Self-check passed — they are ready on the canvas.',
    addedSomeFailed: '{{failed}} did not pass the self-check — still usable; here is what happened.',
    selfCheckMeaning: 'The self-check is Nomi calling the model once. Failing usually means we guessed this provider’s API wrong, not that the model is broken.',
    failedBadge: 'Self-check failed',
    why: {
      notFound: 'No such endpoint at this address ({{status}}). The base URL is probably missing or has an extra path segment.',
      ...
      input: 'The provider rejected the request ({{status}}) — the call shape we guessed does not match theirs. Changing the URL or key will not help.',
      ...
      compile: 'We could not understand this provider’s API docs, so no call was generated. Not caused by anything you entered.',
  ...
  comfyPreset: {
    ...
    riskMissing: '{{count}} item(s) still missing — the template is added anyway, but generation will stall there. Click again to enable.',
    riskOffline: 'ComfyUI is offline, so nothing can be checked — enable it now if you like, just start ComfyUI before generating.',
  ...
  comfyTemplates: {
    ...
    riskMissing: '{{count}} item(s) still missing — the template is added anyway, but generation will stall there. Click again to add.',
    riskOffline: 'ComfyUI is offline, so nothing can be checked — add it now if you like, just start ComfyUI before generating.',
  ...
  customCall: {
    ...
    configLabel: '② Does this service need another key or setting?',
    configLabelFilled: '② Custom config · {{count}}',
    configHint: 'Nomi only has one key slot. Some services also want a second key, a region, or an api version — add them here and read them in the script as config.name',
    ...
    vars: {
      ...
      config: 'key/value pairs you entered under "custom config" (second secret / region / api version — read as config.name)',
      ...
      saveFile: 'write raw bytes into the project and get a URL back (use when the upstream returns a binary body; do not build a huge data URL for video)',
      sleep: 'delay helper',
      signal: 'cancellation signal',
    },
    testRun: 'Test run once (spends one minimal credit)',
    testInputLabel: 'Test a real mode (optional)',
    testPromptPlaceholder: 'Test prompt; leave empty for the low-cost default',
    testPromptAria: 'Custom call test prompt',
    testParamsPlaceholder: '{\n  "first_frame_url": "https://…",\n  "reference_image_urls": ["https://…"]\n}',
    testParamsAria: 'Custom call test params JSON',
    ...
    limitNote: 'Two cases this route cannot cover: services that only notify via callback (your machine has no public address to receive it), and services that ship an SDK but no HTTP docs (a script cannot install packages).',
    saveFailed: 'Save failed: {{message}}',
