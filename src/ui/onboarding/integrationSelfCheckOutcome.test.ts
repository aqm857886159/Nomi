...
describe('integrationSelfCheckOutcome', () => {
  ...
  it('still reports a failure that arrives without a reason code', () => {
    expect(integrationSelfCheckOutcome({ stage: 'failed' })).toEqual({ done: false, reasonCode: null })
  })

  it('closes the panel on every non-failed stage', () => {
    for (const stage of ['certifying', 'completed', 'partial', 'awaiting_confirmation']) {
      expect(integrationSelfCheckOutcome({ stage })).toEqual({ done: true })
    }
  })

  it('closes the panel when main returns no projection at all', () => {
    expect(integrationSelfCheckOutcome(null)).toEqual({ done: true })
    expect(integrationSelfCheckOutcome('nope')).toEqual({ done: true })
  })
})
