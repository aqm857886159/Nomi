...
export type PlanShot = {
  ...
  modelKey?: string
  /**
   * 该镜所选模型的**供应商** key。模型身份的唯一键是 `(vendor, modelKey)`——同名模型来自不同
   * 供应商是两个模型。缺此字段时落画布只能按 modelKey 反查、命中目录里第一家，与用户所选无关
   * （2026-08-18 登记、2026-09-03 真实付费走查实测阻断：选 APIMart 却发去 code-newcli-com）。
   */
  modelVendor?: string
