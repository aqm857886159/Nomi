...
describe('T8 集成：connect_nodes 按目标模型能力校验参考边（真实 store + 真实档案）', () => {
  ...
  it('① 文本节点→图片节点：作为 prompt 上下文边放行，入 store', async () => {
    ...
    expect(useGenerationCanvasStore.getState().edges).toMatchObject([{ mode: 'reference' }])
  })
  ...
  it('手动连线(connectToNode)同样按能力校验——文本→图片 prompt 上下文放行、图片→视频首帧放行', () => {
    const store = useGenerationCanvasStore.getState()
    // 文本→图片:作为 prompt 上下文边放行,不要求文本产出参考 URL。
    ...
    const textEdge = useGenerationCanvasStore.getState().connectToNode(img.id)
    expect(textEdge.ok).toBe(true)
  ...
  it('混合一批：好边连、坏边拒——一次 create 里分流正确', async () => {
    const result = (await applyCanvasToolCall('create_canvas_nodes', {
      ...
      edges: [
        ...
        { sourceClientId: 'x_txt', targetClientId: 'x_vid', mode: 'reference' }, // 好：prompt 上下文
      ],
    })) as CreateResult
    expect(result.connectedCount).toBe(3)
    expect(result.skippedEdges ?? []).toHaveLength(0)
    const modes = useGenerationCanvasStore.getState().edges.map((e) => e.mode).sort()
    expect(modes).toEqual(['character_ref', 'first_frame', 'reference'])
  })
})
