...
export function ProductionContractSummary({ view }: { view: ProductionContractView }): JSX.Element {
  ...
  const specValues = [
    view.specs.durationSeconds === null
      ? null
      : t('generationCommon.production.contract.durationValue', { count: view.specs.durationSeconds }),
    view.specs.aspectRatio,
    view.specs.language,
    view.specs.shotCount === null
      ? null
      : t('generationCommon.production.contract.shotValue', { count: view.specs.shotCount }),
  ]
  ...
  return (
    <div className={cn('grid gap-4')} data-production-contract-summary>
      <div className={cn('grid grid-cols-4 border-y border-nomi-line-soft')}>
        {specLabels.map((label, index) => (
          <div key={label} className={cn('min-w-0 border-r border-nomi-line-soft px-3 py-2.5 last:border-r-0')}>
            <div className={cn('text-micro text-nomi-ink-40')}>
              {t(`generationCommon.production.contract.${label}`)}
            </div>
      ...
      <section className={cn('grid gap-2')}>
        <h3 className={cn('text-caption font-semibold text-nomi-ink')}>
          {t('generationCommon.production.contract.claims')}
        </h3>
        {view.claims.length ? (
          view.claims.map((claim) => (
            <div
              key={claim.text}
              className={cn(
                'grid grid-cols-[auto_1fr_auto] items-start gap-2 border-b border-nomi-line-soft py-2 last:border-b-0',
              )}
            >
              {claim.verified ? (
          ...
          <div className={cn('mt-1 flex flex-wrap gap-1.5')}>
            {view.skills.length ? (
              view.skills.map((skill) => (
                <code key={`${skill.name}@${skill.version}`} className={cn('text-caption text-nomi-ink-80')}>
                  {skill.name}@{skill.version}
                </code>
              ))
            ) : (
              <span className={cn('text-caption text-nomi-ink-40')}>
                {t('generationCommon.production.contract.noneRecorded')}
              </span>
            )}
          </div>
        </div>
        <div>
          ...
          <div className={cn('mt-1 grid gap-1')}>
            {view.providerModels.length ? (
              view.providerModels.map((item) => (
                <span key={`${item.provider}:${item.model}`} className={cn('truncate text-caption text-nomi-ink-80')}>
        ...
        <div className={cn('shrink-0 text-right')}>
          ...
          >
            {view.cost.hardLimit === null ? (
              <>
                <IconAlertTriangle size={15} stroke={1.7} aria-hidden />
                {t('generationCommon.production.contract.notSet')}
              </>
            ) : (
