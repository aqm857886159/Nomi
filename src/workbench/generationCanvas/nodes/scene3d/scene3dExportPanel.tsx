...
/** P0-4：出片后右下角产物卡片（生成中提示，20s 自动收起由宿主控制） */
export function Scene3DExportingCard({ open, onDismiss }: { open: boolean; onDismiss: () => void }): JSX.Element | null {
  if (!open) return null
  return (
    <div className="fixed bottom-6 right-6 z-[59] flex items-center gap-3 rounded-nomi border border-[var(--nomi-line-soft)] bg-[var(--workbench-surface-solid)] px-4 py-3 shadow-workbench-pop">
      <div className="flex items-center gap-2">
        <div className="size-2 animate-pulse rounded-full bg-[var(--nomi-accent)]" />
        <span className="text-caption font-medium text-[var(--workbench-ink)]">参考视频生成中…</span>
      </div>
      <span className="text-caption text-[var(--workbench-muted)]">完成后自动进画布</span>
      <button
        type="button"
        onClick={onDismiss}
