  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        ...
        >
          {t('scene3d.trajectory.controlPoint')}
        </button>
        <button
          ...
          type="button"
          title={t('scene3d.trajectory.deleteControlPoint')}
        ...
        <>
          <div className="grid grid-cols-2 gap-2">
            <label className="grid gap-1">
              <span className="text-micro text-[var(--nomi-ink-60)]">{t('scene3d.trajectory.start')}</span>
              <input
            ...
            <label className="grid gap-1">
              <span className="text-micro text-[var(--nomi-ink-60)]">{t('scene3d.trajectory.end')}</span>
              <input
                className="h-8 min-w-0 rounded-nomi-sm border border-[var(--nomi-line)] bg-[var(--nomi-paper)] px-2 text-caption text-[var(--nomi-ink)] outline-none"
                disabled={readOnly}
                min={0.1}
          ...
          <label className="grid gap-1">
            <span className="text-micro text-[var(--nomi-ink-60)]">{t('scene3d.trajectory.direction')}</span>
            <NomiSelect
              ariaLabel={t('scene3d.trajectory.direction')}
              disabled={readOnly}
              value={binding.direction}
              options={[
                { value: 'forward', label: t('scene3d.trajectory.forward') },
                { value: 'reverse', label: t('scene3d.trajectory.reverse') },
              ]}
          ...
          {binding.objects.map((boundObject) => (
            <div key={boundObject.objectId} className="grid grid-cols-[minmax(0,1fr)_72px_28px] items-end gap-1">
              ...
              <label className="grid gap-1">
                <span className="text-micro text-[var(--nomi-ink-40)]">{t('scene3d.trajectory.offset')}</span>
                <input
              ...
              <button
                ...
                disabled={readOnly}
                type="button"
                title={t('scene3d.trajectory.unbindNode')}
          ...
          <NomiSelect
            ariaLabel={t('scene3d.trajectory.addBoundNode')}
            placeholder={t('scene3d.trajectory.addBoundNode')}
          ...
          >
            {t('scene3d.trajectory.deleteBinding')}
          </button>
        </>
      ) : (
        <NomiSelect
          ariaLabel={t('scene3d.trajectory.selectNodeToBind')}
          placeholder={t('scene3d.trajectory.selectNodeToBind')}
          disabled={readOnly || availableNodes.length === 0}
          value=""
          options={availableNodes.map((node) => ({ value: node.id, label: node.name }))}
          onChange={(objectId) => {
  ...
  return (
    <section className="min-h-0 flex-1 overflow-auto bg-[var(--nomi-paper)] px-3 py-3">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="text-caption font-medium text-[var(--nomi-ink)]">{t('scene3d.trajectory.properties')}</div>
        <button
        ...
        >
          <IconPlus size={14} />
          <span>{t('scene3d.trajectory.create')}</span>
        </button>
      </div>
      ...
      {!activeTrajectory ? (
        <div className="rounded-nomi-sm border border-dashed border-[var(--nomi-line)] px-3 py-4 text-center text-caption text-[var(--nomi-ink-40)]">
          {t('scene3d.trajectory.selectTrajectory')}
        </div>
      ) : (
        <div className="grid gap-3">
          ...
          <label className="grid gap-1">
            <span className="text-micro text-[var(--nomi-ink-60)]">{t('scene3d.trajectory.name')}</span>
            <input
          ...
          <div className="grid grid-cols-[32px_minmax(0,1fr)] items-end gap-2">
            <label className="grid gap-1">
              <span className="text-micro text-[var(--nomi-ink-60)]">{t('scene3d.trajectory.color')}</span>
              <input
            ...
            <label className="grid gap-1">
              <span className="text-micro text-[var(--nomi-ink-60)]">{t('scene3d.trajectory.tension')}</span>
              <input
          ...
          <label className="flex items-center justify-between gap-2 rounded-nomi-sm border border-[var(--nomi-line-soft)] bg-[var(--nomi-ink-05)] px-2 py-2 text-caption text-[var(--nomi-ink-60)]">
            <span>{t('scene3d.trajectory.closed')}</span>
            <input
          ...
          <div className="grid gap-2">
            <div className="text-caption font-medium text-[var(--nomi-ink)]">{t('scene3d.trajectory.currentBinding')}</div>
          ...
          <div className="grid gap-2">
            <div className="flex items-center justify-between gap-2">
              <div className="text-caption font-medium text-[var(--nomi-ink)]">{t('scene3d.trajectory.controlPoint')}</div>
              <button
              ...
              >
                <IconPlus size={13} />
                <span>{t('scene3d.trajectory.appendPoint')}</span>
              </button>
            </div>
          ...
          >
            {t('scene3d.trajectory.deleteTrajectory')}
          </button>
        </div>
      )}
    </section>
