...
import { normalizeLegacyScene3D } from '../migration/legacyScene3dTypes'
import { auditStagingSpec, buildStagingScene, buildStagingSceneAudited, resolveStagingPose } from './stagingBuilder'

describe('buildStagingScene', () => {
  it('solo: 1 个 mannequin + 1 机位，落地高度正确，容错读取不丢', () => {
    ...
    const normalized = normalizeLegacyScene3D(state)
    expect(normalized.objects).toHaveLength(1)
    expect(normalized.cameras).toHaveLength(1)
  })

  it('facing 双人：两人对向（rotation.y 反号），机位在前方', () => {
    const state = buildStagingScene({ characters: [{ name: '甲', pose: 'single-knee' }, { name: '乙', pose: 'standing' }], layout: 'facing', camera: { angle: 'front', height: 'low', shot: 'medium' } })
    const [a, b] = state.objects
    expect(a.name).toBe('甲')
    expect(b.name).toBe('乙')
    expect(Math.sign(a.rotation[1])).toBe(-Math.sign(b.rotation[1]))
    expect(a.pose).toBeDefined()
  ...
  it('环境预设：night 暗色背景、studio 关网格', () => {
    expect(buildStagingScene({ characters: [{}], environment: 'night' }).environment.backgroundColor).toBe('#161c28')
    expect(buildStagingScene({ characters: [{}], environment: 'studio' }).environment.showGrid).toBe(false)
  })

  it('标签缺省自动编号 角色A/B，facing override 生效', () => {
    const state = buildStagingScene({ characters: [{ facing: 'camera' }, { facing: 'away' }], layout: 'facing' })
    expect(state.objects[0].name).toBe('角色A')
    expect(state.objects[1].name).toBe('角色B')
    expect(state.objects[0].rotation[1]).toBeCloseTo(0, 5)
    expect(Math.abs(state.objects[1].rotation[1])).toBeCloseTo(Math.PI, 5)
  })
  ...
  it('sceneTemplate: 街道布景铺在角色下、角色仍是唯一 mannequin，机位仍取角色', () => {
    ...
    expect(withStreet.objects.length).toBeGreaterThan(plain.objects.length)
    ...
    expect(normalizeLegacyScene3D(withStreet).objects.length).toBe(withStreet.objects.length)
  })

  it('props: 显式位置 / 朝向 / 缩放生效，省略位置自动铺开不与角色堆叠；未知 kind 被丢弃', () => {
    const state = buildStagingScene({
      characters: [{ pose: 'standing' }],
      // @ts-expect-error 故意传非法 kind
      props: [{ kind: 'car', position: [3, -1], rotationY: 90, scale: 1.2 }, { kind: 'tree' }, { kind: 'tree' }, { kind: 'spaceship' }],
    })
...
describe('运行时自检：姿势 id 解析', () => {
  it('词表 id 原样通过、无 pose = 站立（合法，无 note）', () => {
    expect(resolveStagingPose('single-knee')).toEqual({ id: 'single-knee' })
    expect(resolveStagingPose('crouch')).toEqual({ id: 'crouch' })
    expect(resolveStagingPose(undefined)).toEqual({})
    expect(resolveStagingPose('')).toEqual({})
  })
  it('别名 / 近似归一到词表 id 并带 note', () => {
    expect(resolveStagingPose('kneel').id).toBe('single-knee')
    expect(resolveStagingPose('sitting').id).toBe('sit')
    expect(resolveStagingPose('squatting').id).toBe('squat')
    expect(resolveStagingPose('crouching').id).toBe('crouch')
    expect(resolveStagingPose('Kneeling').id).toBe('single-knee')
    expect(resolveStagingPose('hands on hips').id).toBe('hands-on-hips')
...
describe('运行时自检：角色过近自动拉开', () => {
  it('正常间距不触发拉开（无 issue）', () => {
    ...
    expect(Math.hypot(men[0].position[0] - men[1].position[0], men[0].position[2] - men[1].position[2])).toBeGreaterThanOrEqual(1.0)
    expect(issues.filter((i) => i.includes('间距'))).toHaveLength(0)
  })
  it('近景面对面也保证中心间距 ≥ 下限（必要时自动加宽）', () => {
    const { state } = buildStagingSceneAudited({ characters: [{ pose: 'standing', facing: 'toward' }, { pose: 'standing', facing: 'toward' }], layout: 'facing', camera: { shot: 'close' } })
    const men = state.objects.filter((o) => o.type === 'mannequin')
    expect(Math.hypot(men[0].position[0] - men[1].position[0], men[0].position[2] - men[1].position[2])).toBeGreaterThanOrEqual(1.0)
  })
})
