...
describe('canvas add menu preferences', () => {
  it('saves hidden/order through the settings bridge, reloads, and restores every default item', async () => {
    ...
    expect(canvasResidentAddIntents(preference).map((item) => item.id)).toEqual(['video', 'image', 'clip', 'import-file'])
    ...
    expect(canvasResidentAddIntents(useCanvasMenuPreferenceStore.getState().preference)).toHaveLength(5)
  })
