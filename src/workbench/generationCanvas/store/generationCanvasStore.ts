...
import { getHistoryFlags, popRedo, popUndo, pushUndoSnapshot, seedUndoJournalBase } from '../events/canvasUndoJournal'
import {
...
export const useGenerationCanvasStore = create<GenerationCanvasState>()(
  subscribeWithSelector(
    immer((set, get, store) =>
      withCanvasWriteBoundary({
        isReady: false,
        persistRevision: 0,
        ...
        pasteNodes: (basePosition) => {
          ...
          const relevantExisting = currentState.nodes.filter((node) =>
            clusterCategories.has(node.categoryId || 'shots'),
          )
          ...
          emitCanvasGesture([
            {
              ...
              payload: { snapshot: { nodes: previous.nodes, edges: previous.edges, groups: previous.groups } },
            },
          ])
        },
        redo: () => {
          ...
          emitCanvasGesture([
            {
              type: 'canvas.snapshot.restored',
              payload: { snapshot: { nodes: next.nodes, edges: next.edges, groups: next.groups } },
            },
          ])
        },
        readSnapshot: () => {
        ...
        applyExternalGraph: (snapshot) => {
          ...
          emitCanvasGesture([
            {
              type: 'canvas.snapshot.restored',
              payload: { snapshot: { nodes: normalized.nodes, edges: normalized.edges, groups: normalized.groups } },
            },
          ])
        },
