...
export function ResidentToolChips({ items, emptyLabel, statusLabel, sectionLabel, headerLabel, explanationLabel, targetLabel, resultLabel, technicalLabel }: { items: readonly ResidentToolChipData[]; emptyLabel: string; statusLabel: (status: ProjectAgentStatus) => string; sectionLabel: string; headerLabel: string; explanationLabel: string; targetLabel: string; resultLabel: string; technicalLabel: string }): JSX.Element | null {
  ...
  // data-agent-tool-line: spec §0 挂点（保留 data-agent-tool-chips 兼容外部引用）
  return <section className="space-y-1.5" data-agent-tool-chips="true" data-agent-tool-line="true" aria-label={sectionLabel}>
  ...
  return <section className="space-y-1 border-t border-nomi-line-soft px-3 py-1.5" data-agent-queue="true">
    <div className="flex min-w-0 items-center justify-between gap-2 text-micro text-nomi-ink-60">
     ...
     {open ? <ul id="agent-queue-items" className="m-0 grid max-h-40 list-none gap-1 overflow-y-auto overscroll-contain p-0">{visibleEntries.map((entry) => {
       ...
       return <li key={entry.queueItemId} className="flex min-h-7 min-w-0 flex-wrap items-center gap-1.5 rounded-nomi-sm bg-nomi-ink-05 px-2 py-1 text-micro" data-agent-queue-item="true" data-agent-queue-row="true" data-agent-queue-paused={paused ? 'true' : undefined}>
         ...
         </span> : <button type="button" data-agent-queue-action="cancel" data-agent-queue-remove="true" className="ml-auto grid size-7 shrink-0 place-items-center rounded-nomi-sm text-nomi-ink-60 hover:bg-workbench-danger-soft hover:text-workbench-danger" aria-label={cancelLabel} title={cancelLabel} onClick={() => onCancel(entry)}><IconX size={13} aria-hidden="true" /></button>}
       </li>
     })}</ul> : null}
  </section>
}
