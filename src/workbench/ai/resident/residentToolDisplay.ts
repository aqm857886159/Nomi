...
export function readableToolDetailRows(t: Translate, name: string, args?: unknown): readonly ResidentApprovalDetail[] {
  ...
  rows.push({ label: t('agentResident.toolTargetLabel'), value: readableToolTarget(t, name, record) })
  ...
  if (isCanvasWriteToolName(name)) rows.push({ label: t('agentResident.toolBoundaryLabel'), value: t('agentResident.toolNoGeneration') })
  const patch = asRecord(record.patch)
  const candidate = asRecord(record.candidate)
  ...
  if (modelValues.length) rows.push({ label: t('agentResident.toolModelLabel'), value: Array.from(new Set(modelValues)).join(' · ') })
  ...
  if (promptValues.length) rows.push({ label: t('agentResident.toolPromptLabel'), value: Array.from(new Set(promptValues)).join(' · ') })
  if (!nodes.length && Array.isArray(record.shots) && record.shots.length) rows.push({ label: t('agentResident.toolShotLabel'), value: t('agentResident.targetShotCount', { count: record.shots.length }) })
  ...
  if (isGenerationToolName(name) || normalized.includes('canvas.write')) rows.push({ label: t('agentResident.proposalEstimate'), value: readableEstimate(t, record) })
