...
/**
 * 五档**注意力状态**。注意它们各自的注释说的是「宿主那边发生了什么」，**不是「角标长什么样」**：
 * 定稿 §11.2 只给了两种长相（蓝点 / 数字），由 `agentTopbarChipBadge()` 从未读 + 待决算出来，
 * 跟这五个词没有一一对应关系。这五个词只进 tooltip 那句人话与 `data-agent-dock-status`。
 *
 * （这条注释本身是 2026-09-07 修的：它原来写着「done = 短暂一个勾」「failed = 警示角标」，
 * 而那两种图形从来没被实现过——一份描述不存在长相的注释，就是下一个人照着画出第三、第四种
 * 图形的入口，也正是 R14.1 要横扫的「同一件事两份定义」。）
 */
export type V4DockStatus =
  ...
  /** 有介入槽在等用户裁决。 */
  | 'needs-confirm'
  /** 刚跑完（由 `useV4DockStatus` 掐 `V4_DOCK_DONE_HOLD_MS` 的表，然后自己落回 idle）。 */
  | 'done'
  /** 最后一件事是失败。 */
  | 'failed'

/** `done` 这一档停留多久（之后落回 `idle`）。够 tooltip 说完那句「刚做完」，不够变成一块常驻装饰。 */
