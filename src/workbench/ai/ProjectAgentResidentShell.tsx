...
import {
  ...
  IconHistory, IconLayoutSidebarRightCollapse, IconListCheck, IconLoader2, IconLock, IconMicrophone, IconNotes, IconPaperclip, IconPencil, IconPhoto,
  IconMessageQuestion, IconRoute, IconSearch, IconSettings, IconStack2, IconTextSpellcheck, IconTimelineEvent, IconWaveSine,
  IconPlayerStopFilled, IconPlus, IconSparkles, IconTrash, IconVideo, IconWorld, IconWorldSearch, IconX,
...
function MenuRow({ children, onClick, onMouseEnter, onFocus, selected, disabled, testId, className }: { children: React.ReactNode; onClick?: () => void; onMouseEnter?: () => void; onFocus?: () => void; selected?: boolean; disabled?: boolean; testId?: string; className?: string }): JSX.Element {
  ...
  return <button type="button" disabled={disabled} data-agent-menu-item={testId} onClick={() => { if (pointerActivated.current) { pointerActivated.current = false; return }; onClick?.() }} onPointerDown={(event) => { event.stopPropagation(); if (event.button === 0 && !disabled) { pointerActivated.current = true; onClick?.() } }} onMouseEnter={onMouseEnter} onFocus={onFocus} className={cn('flex min-h-10 w-full items-center gap-2 rounded-nomi-sm px-2 py-1.5 text-left text-caption transition-colors', selected ? 'bg-nomi-accent-soft text-nomi-accent' : 'hover:bg-nomi-ink-05', disabled && 'cursor-not-allowed opacity-45', className)}>{children}</button>
}

function iconControlClass(active = false): string {
  return cn('inline-grid size-7 shrink-0 place-items-center rounded-nomi-sm border p-0 transition-colors', active ? 'border-nomi-accent bg-nomi-accent-soft text-nomi-accent' : 'border-nomi-line bg-transparent text-nomi-ink-70 hover:bg-nomi-ink-05')
}

function MenuCopy({ label, hint }: { label: React.ReactNode; hint?: React.ReactNode }): JSX.Element {
  return <span className="min-w-0 flex-1"><span className="block truncate">{label}</span>{hint ? <span className="mt-0.5 block truncate text-micro text-nomi-ink-40">{hint}</span> : null}</span>
}
...
export default function ProjectAgentResidentShell({ surface }: { surface: ResidentSurface }): JSX.Element {
  ...
  if (collapsed) {
    return <section id="project-agent-resident" className="relative h-full w-full overflow-visible" aria-label={t('agentResident.aria')} data-agent-resident="true" data-agent-surface={surface}>
      <button type="button" className="absolute right-0 top-0 z-40 flex h-9 w-[min(230px,calc(100vw-24px))] max-w-[230px] items-center gap-2 rounded-pill border border-nomi-line bg-nomi-paper px-2.5 text-left text-caption text-nomi-ink shadow-nomi-md transition-[box-shadow,transform] hover:-translate-y-px hover:shadow-nomi-lg" aria-label={t('agentResident.expand')} title={t('agentResident.expand')} aria-controls="project-agent-resident" aria-expanded="false" data-agent-resident-collapsed="true" onClick={() => setCollapsed(false)}>
        ...
        <span className="min-w-0 flex-1 truncate">{compactStatus}</span>
  ...
  return <section id="project-agent-resident" onKeyDownCapture={(event) => { if (event.key === 'Escape') { setThreadsOpen(false); setMenu(null) } }} className="relative isolate flex h-full min-h-0 w-full min-w-0 flex-col bg-[var(--workbench-ai-panel-bg)] text-nomi-ink" aria-label={t('agentResident.aria')} data-agent-resident="true" data-agent-surface={surface}>
    <header className="relative flex shrink-0 items-center gap-2 border-b border-nomi-line-soft px-3 py-2" data-agent-header="true">
      <div className="flex min-w-0 flex-1 items-center gap-2 text-left"><NomiLogoMark size={19} /><span className="min-w-0"><span className="block text-body-sm font-semibold leading-tight">{t('agentResident.brand')}</span><span className="block truncate text-micro text-nomi-ink-50">{activeThread?.title || t('agentResident.untitledThread')}</span></span></div>
      <span className="flex shrink-0 items-center gap-1 text-micro text-nomi-ink-50" aria-label={t('agentResident.usage', { value: 0 })}><span className="h-1 w-6 overflow-hidden rounded-pill bg-nomi-ink-10"><span className="block h-full w-0 rounded-pill bg-nomi-ink-40" /></span><span>0%</span></span>
      ...
      {threadsOpen ? <div ref={threadMenuRef} tabIndex={-1} onKeyDown={(event) => { if (event.key === 'Escape') setThreadsOpen(false) }} className="absolute right-2 top-full z-50 mt-1 w-[280px] rounded-nomi border border-nomi-line bg-nomi-paper p-1.5 shadow-nomi-lg" data-agent-thread-menu="true" role="menu"><div className="flex items-center justify-between px-2 py-1 text-micro text-nomi-ink-50"><span>{t('agentResident.threads')}</span><button type="button" className="text-nomi-accent" onClick={() => { void createProjectAgentThread(); setThreadsOpen(false) }}>{t('agentResident.newThread')}</button></div>{(snapshot?.threads ?? []).map((thread) => <div key={thread.threadId} className={cn('flex items-center gap-1 rounded-nomi-sm px-2 py-1.5', thread.threadId === activeThreadId && 'bg-nomi-accent-soft')}><button type="button" className="min-w-0 flex-1 truncate text-left text-caption" onClick={() => { void activateProjectAgentThread(thread.threadId); setThreadsOpen(false) }}>{thread.title || t('agentResident.untitledThread')}</button><button type="button" className="grid size-6 place-items-center rounded-nomi-sm hover:bg-nomi-ink-10" aria-label={t('agentResident.removeThread')} onClick={() => void removeProjectAgentThread(thread.threadId)}><IconTrash size={13} /></button></div>)}</div> : null}
    </header>
    <div className="flex shrink-0 items-center justify-between gap-2 border-b border-nomi-line-soft px-3 py-2" data-agent-context="true"><div className="min-w-0 truncate text-caption"><span className="text-nomi-ink-50">{t('agentResident.currentScene')} · </span><span className="font-medium">{surfaceShortLabel(t, surface)}</span><span className="text-nomi-ink-50"> · {contextCount}</span></div><button type="button" className="inline-flex h-7 shrink-0 items-center gap-1 rounded-nomi-sm px-1.5 text-micro font-medium text-nomi-ink-70 hover:bg-nomi-ink-05" onClick={focusContext}>{t('agentResident.backToScene')}<IconChevronRight size={12} /></button></div>
    <div ref={scrollRef} className={cn('min-h-0 flex-1 space-y-2 overflow-y-auto px-3 py-3', menu && 'pointer-events-none')} role="log" aria-live="polite" data-agent-transcript="true">
      {!items.length && !queue.length ? <div className="grid min-h-40 place-items-center px-3 text-center"><div><div className="mb-1 text-body-sm font-semibold">{t('agentResident.emptyTitle')}</div><p className="m-0 text-caption text-nomi-ink-60">{t('agentResident.emptyDescription')}</p></div></div> : null}
      {items.map((item) => { const proposal = item.kind === 'proposal' && item.approval; return <article key={item.itemId} data-agent-item-kind={item.kind} className={cn('rounded-nomi-sm border px-2.5 py-2 text-caption', item.kind === 'user' ? 'ml-6 border-nomi-ink bg-nomi-ink text-nomi-paper' : item.kind === 'failure' ? 'border-workbench-danger bg-workbench-danger-soft' : proposal ? 'border-nomi-accent bg-nomi-accent-soft' : 'border-nomi-line-soft bg-nomi-paper')}>
        ...
        {item.kind === 'proposal' ? <><div className="flex items-center gap-1.5 font-medium"><IconListCheck size={18} className="text-nomi-accent" />{t('agentResident.plan')}<span className="ml-auto text-micro text-nomi-accent">{statusLabel(t, item.status)}</span></div><div className="mt-1 text-micro text-nomi-ink-70">{t('agentResident.approval')}</div>{proposal ? <div className="mt-2 grid grid-cols-2 gap-1.5"><button type="button" className="h-7 rounded-nomi-sm border border-nomi-line text-micro" data-agent-action="edit-plan" onClick={() => setDraft(t('agentResident.editPlanPrompt'))}>{t('agentResident.editPlan')}</button><button type="button" className="h-7 rounded-nomi-sm bg-nomi-ink text-micro text-nomi-paper" data-agent-action="approve-plan" disabled>{t('agentResident.approve')}</button></div> : null}</> : null}
        {item.kind === 'failure' ? <><div className="flex items-center gap-1.5 font-medium text-workbench-danger"><IconAlertTriangle size={18} />{item.message}</div><div className="mt-1 text-micro text-nomi-ink-70">{item.nextAction ?? t('agentResident.sendFailed')}</div><div className="mt-2 grid grid-cols-3 gap-1.5"><button type="button" className="h-7 min-w-0 rounded-nomi-sm bg-nomi-ink px-1 text-micro text-nomi-paper" data-agent-action="retry" onClick={() => { const user = items.find((candidate) => candidate.kind === 'user' && candidate.turnId === item.turnId); if (user?.kind === 'user') setDraft(user.text) }}>{t('agentResident.changeModelRetry')}</button><button type="button" className="h-7 min-w-0 rounded-nomi-sm border border-nomi-line px-1 text-micro" data-agent-action="edit-prompt" onClick={() => setDraft(t('agentResident.editPlanPrompt'))}>{t('agentResident.editPrompt')}</button><button type="button" className="h-7 min-w-0 rounded-nomi-sm border border-nomi-line px-1 text-micro" data-agent-action="view-log" onClick={() => window.dispatchEvent(new Event('nomi-open-task-center'))}>{t('agentResident.viewLog')}</button></div></> : null}
