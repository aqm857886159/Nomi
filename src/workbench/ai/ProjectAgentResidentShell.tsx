...
import { TimelineSelectionChips } from './resident/TimelineSelectionChips'
import { TimelineAgentReceiptEffect } from './resident/TimelineAgentReceiptEffect'
import { ResidentCollapsedDock } from './resident/ResidentCollapsedDock'
import { useResidentActivityStore } from './residentActivity'
import { useTimelinePlanPreview, useTimelinePlanRows, useTimelineSelectionChips } from './resident/timelineAgentSurface'
...
export default function ProjectAgentResidentShell({ surface }: { surface: ResidentSurface }): JSX.Element {
  ...
  const toolChipItems = React.useMemo<ResidentToolChipData[]>(() => items.filter((item): item is Extract<ProjectAgentItem, { kind: 'tool' }> => item.kind === 'tool' && item.capability.id !== 'skill.read').map((item) => {
    ...
    return { id: item.itemId, label: readableToolName(t, item.capability.id, args), name: item.capability.id, effect, target, summary: effect, detail: technicalDetails, technicalDetails, result, status: item.status, createdAt: item.createdAt, updatedAt: item.updatedAt }
  ...
  // Thread commands used to be fired as bare `void promise` from the menu, so a Host refusal became
  // an unhandled rejection and the user saw nothing at all. They go through the same error banner as
  // every other command now.
  const runThreadCommand = React.useCallback((command: () => Promise<unknown>) => {
  ...
  // 收起后那颗状态点读的是同一份判据、同一格 token（residentActivity 只是它的投影口，
  // 图标条不在这棵子树里，够不着 runningTurn / pendingTools）。
  const activityDotClassName = runningTurn
    ? 'bg-nomi-accent'
    : pendingTools.some((pending) => pending.state === 'pending') ? 'bg-nomi-warning' : 'bg-nomi-ink-30'
  const publishActivity = useResidentActivityStore((state) => state.setResidentActivity)
  React.useEffect(() => { publishActivity(activityDotClassName, compactStatus) }, [activityDotClassName, compactStatus, publishActivity])
  ...
  const timelinePlanPreviewPortal = useTimelinePlanPreview(surface, pendingTools, timeline, t('agentResident.timelinePlanPreview'))
  const timelinePlanRows = useTimelinePlanRows(primaryPending?.call.toolName, interventionEditableArgs, timeline, t)
  const interventionTitle = interventionKind === 'question' ? t('agentResident.questionPage') : interventionKind === 'missing_credential' ? t('agentResident.missingCredentialTitle') : interventionKind === 'missing_param' ? t('agentResident.missingParamTitle') : primaryPending ? readableToolName(t, primaryPending.call.toolName, interventionRecord) : t('agentResident.approvalMode')
  const interventionSummary = interventionKind === 'question' ? String(interventionRecord.question ?? '') : interventionKind === 'missing_credential' ? t('agentResident.missingCredentialSummary') : interventionKind === 'missing_param' ? t('agentResident.missingParamSummary') : timelinePlanRows.length ? timelinePlanRows.map((row) => row.text).join(' · ') : primaryPending ? [readableToolPreview(t, primaryPending.call.toolName, interventionRecord), typeof interventionRecord.reason === 'string' ? interventionRecord.reason.trim() : ''].filter(Boolean).join(' · ') : ''
  ...
  const composer = <div className={cn('relative z-20 shrink-0 bg-nomi-paper', collapsed ? 'rounded-nomi-lg border border-nomi-line shadow-nomi-lg' : 'border-t border-nomi-line-soft')} data-agent-composer="true">
    ...
    <form className="relative grid gap-1 px-3 pb-1.5 pt-1" onSubmit={(event) => { event.preventDefault(); void submit() }} {...attachmentApi.dragHandlers}>
      ...
      <Popover open={menu === 'references'} onClose={closeMenu} label={t('agentResident.mention')} className="w-[360px] max-w-[calc(100vw-24px)]">{assetPool.assets.length || !assetPool.loading ? <ResidentAtPicker assets={assetPool.assets} groups={[]} emptyTitle={t('agentResident.atEmptyTitle')} emptyDescription={t('agentResident.atEmptyDescription')} uploadLabel={t('agentResident.atUpload')} searchLabel={t('agentResident.atSearch')} onPickAsset={(asset) => addReference(buildResidentAssetReference(asset.id, asset.name))} onUpload={() => { window.dispatchEvent(new Event('nomi-open-files-panel')); closeMenu() }} /> : null}<div className="border-t border-nomi-line-soft pt-1">{!assetPool.assets.length && assetPool.loading ? <div className="px-2 py-2 text-micro text-nomi-ink-40">{t('agentResident.planLoading')}</div> : null}<MenuRow testId="canvas" onClick={() => addReference(buildResidentReference('canvas', t('agentResident.referenceCanvas'), { documentId: activeDocumentId, nodeIds: surface === 'generation' ? selectedNodeIds : [], clipIds: [] }))}><IconPhoto size={16} className="shrink-0 text-nomi-ink-60" /><MenuCopy label={t('agentResident.referenceCanvas')} hint={surface === 'generation' && selectedNodeIds.length ? t('agentResident.referenceCanvasHintSelected', { count: selectedNodeIds.length }) : t('agentResident.referenceCanvasHintEmpty')} /></MenuRow><MenuRow testId="document" onClick={() => addReference(buildResidentReference('document', t('agentResident.referenceDocument'), { documentId: activeDocumentId, nodeIds: [], clipIds: [] }))}><IconFileText size={16} className="shrink-0 text-nomi-ink-60" /><MenuCopy label={t('agentResident.referenceDocument')} hint={activeDocumentId ? t('agentResident.referenceDocumentHintActive') : t('agentResident.referenceDocumentHintEmpty')} /></MenuRow><MenuRow testId="preview" onClick={() => addReference(buildResidentReference('preview', t('agentResident.referencePreview'), { documentId: activeDocumentId, nodeIds: [], clipIds: surface === 'preview' ? selectedClipIds : [] }))}><IconVideo size={16} className="shrink-0 text-nomi-ink-60" /><MenuCopy label={t('agentResident.referencePreview')} hint={surface === 'preview' && selectedClipIds.length ? t('agentResident.referencePreviewHintSelected', { count: selectedClipIds.length }) : t('agentResident.referencePreviewHintEmpty')} /></MenuRow><MenuRow testId="timeline" onClick={() => addReference(buildResidentReference('timeline', t('agentResident.referenceTimeline'), { documentId: activeDocumentId, nodeIds: [], clipIds: surface === 'preview' ? selectedClipIds : [] }))}><IconTimelineEvent size={16} className="shrink-0 text-nomi-ink-60" /><MenuCopy label={t('agentResident.referenceTimeline')} hint={surface === 'preview' && selectedClipIds.length ? t('agentResident.referenceTimelineHintSelected', { count: selectedClipIds.length }) : t('agentResident.referenceTimelineHintEmpty')} /></MenuRow><MenuRow testId="browser" onClick={() => addReference(buildResidentReference('browser', t('agentResident.referenceBrowser'), { documentId: activeDocumentId, nodeIds: [], clipIds: [] }))}><IconWorld size={16} className="shrink-0 text-nomi-ink-60" /><MenuCopy label={t('agentResident.referenceBrowser')} hint={t('agentResident.referenceBrowserHint')} /></MenuRow></div></Popover>
    </form>
  </div>
  if (collapsed) {
    return <section id="project-agent-resident" className="pointer-events-none relative h-full w-full overflow-visible" aria-label={t('agentResident.aria')} data-agent-resident="true" data-agent-surface={surface} data-agent-collapsed="true">
      <TimelineAgentReceiptEffect />
      {timelinePlanPreviewPortal}
      <ResidentCollapsedDock>{composer}</ResidentCollapsedDock>
    </section>
  }
  ...
  return <section id="project-agent-resident" onKeyDownCapture={(event) => { if (event.key === 'Escape') { setThreadsOpen(false); setMenu(null); setQueueMenuOpen(null) } if (event.key === 'ArrowUp' && event.target instanceof HTMLTextAreaElement && !draft.trim()) { const candidate = activeQueue.find((entry) => entry.status === 'queued'); const user = candidate ? snapshot?.items.find((item) => item.kind === 'user' && item.turnId === candidate.turnId) : undefined; if (candidate && user?.kind === 'user') { event.preventDefault(); setDraft(user.text); setEditingQueue({ queueItemId: candidate.queueItemId, userItemId: user.itemId }) } } }} className="relative isolate flex h-full min-h-0 w-full min-w-0 flex-col bg-[var(--workbench-ai-panel-bg)] text-nomi-ink" aria-label={t('agentResident.aria')} data-agent-resident="true" data-agent-panel="true" data-agent-surface={surface} data-agent-run-mode={runMode} data-agent-approval-mode={approvalPolicy.mode} data-agent-spend-policy={approvalPolicy.spend}>
    ...
    <header className="relative flex min-h-11 shrink-0 items-center gap-2 border-b border-nomi-line-soft px-3 py-1.5" data-agent-header="true">
      ...
      {threadsOpen ? <div ref={threadMenuRef} tabIndex={-1} onKeyDown={(event) => { if (event.key === 'Escape') setThreadsOpen(false) }} className="absolute right-2 top-full z-50 mt-1 w-[280px] rounded-nomi border border-nomi-line bg-nomi-paper p-1 shadow-nomi-lg" data-agent-thread-menu="true" role="menu"><div className="flex items-center justify-between px-2 py-1 text-micro text-nomi-ink-60"><span>{t('agentResident.threads')}</span><button type="button" className="text-nomi-accent" onClick={() => { runThreadCommand(createProjectAgentThread); setThreadsOpen(false) }}>{t('agentResident.newThread')}</button></div>{(snapshot?.threads ?? []).map((thread) => <div key={thread.threadId} className={cn('flex items-center gap-1 rounded-nomi-sm px-2 py-1', thread.threadId === activeThreadId && 'bg-nomi-accent-soft')}><button type="button" className="min-w-0 flex-1 truncate text-left text-caption" onClick={() => { runThreadCommand(() => activateProjectAgentThread(thread.threadId)); setThreadsOpen(false) }}>{thread.title || t('agentResident.untitledThread')}</button><button type="button" className="grid size-7 place-items-center rounded-nomi-sm hover:bg-nomi-ink-10" aria-label={t('agentResident.removeThread')} onClick={() => runThreadCommand(() => removeProjectAgentThread(thread.threadId))}><IconTrash size={13} /></button></div>)}</div> : null}
    </header>
    ...
    <div className="relative min-h-0 flex-1">
      <div ref={scrollRef} className={cn('h-full min-h-0 space-y-1 overflow-y-auto px-3 py-2', menu && 'pointer-events-none')} role="log" aria-live="polite" data-agent-transcript="true" data-agent-flow="true">
        ...
        {items.map((item) => { const proposal = item.kind === 'proposal' && item.approval; const proposalActive = item.kind === 'proposal' && item.status === 'proposed' && Boolean(proposal); const receipt = item.kind === 'proposal' && item.status === 'done' && item.approval?.receiptProposalId === committedProposal?.proposalId ? committedProposal : null; const declined = item.kind === 'failure' && item.status === 'declined'; const streaming = item.kind === 'assistant' && (isLive(item.status) || runningTurn?.turnId === item.turnId); const failureCode = item.kind === 'failure' ? safeAgentFailureCode(item.code) : undefined; const failureCategory = item.kind === 'failure' ? agentFailureCategory(item.code, item.message) : undefined; if (item.kind === 'tool' || proposalActive) return null; return <article key={item.itemId} data-agent-item-kind={item.kind} data-agent-turn-id={item.turnId} data-agent-status={item.status} data-agent-user-bubble={item.kind === 'user' ? 'true' : undefined} data-agent-error-code={failureCode} data-agent-error-message-category={failureCategory} className={residentItemClassName(item, declined)}>
          {item.kind === 'user' ? <ResidentFoldableText text={item.text} expandLabel={t('agentResident.foldMore')} collapseLabel={t('agentResident.foldCollapse')} estimatedExtra={t('agentResident.foldCharacters', { count: item.text.length })} dataUserContent foldLinkOutside className="text-caption" contentWrapClassName="rounded-nomi-sm border border-nomi-ink bg-nomi-ink px-3 py-2" contentClassName="text-nomi-paper" /> : null}
          {item.kind === 'assistant' ? <><div data-agent-reply="true" className="block max-w-full" title={item.text.length > 360 ? item.text : undefined}><ResidentFoldableText text={item.text || (streaming ? `${t('creationAi.assistantMessage.processing')}…` : '')} expandLabel={t('agentResident.foldMore')} collapseLabel={t('agentResident.foldCollapse')} estimatedExtra={t('agentResident.foldCharacters', { count: item.text.length })} contentClassName="text-caption leading-5" />{streaming ? <span data-agent-stream-cursor="true" className="ml-0.5 inline-block h-[1em] w-px translate-y-[0.1em] rounded-pill bg-nomi-accent align-baseline motion-safe:animate-pulse motion-reduce:animate-none" aria-hidden="true" /> : null}</div>{item.status === 'stopped' ? <span data-agent-status-label="stopped" className="text-micro text-nomi-ink-40">{statusLabel(t, item.status)}</span> : null}</> : null}
