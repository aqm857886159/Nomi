...
// 存**整键**而非 `agentResident.${key}` 拼接：拼接的模板 head 覆盖整棵 agentResident 命名空间（358 叶），
// 死键门岗对它整片失明（见 src/i18n/translationKey.ts）；整键还让编译器替我们校验键存在。
...
export default function ProjectAgentResidentShell({ surface }: { surface: ResidentSurface }): JSX.Element {
  ...
  const promptTitle = selectedLibraryPrompt
    ...
    : promptModeId === 'general'
      ...
      : `${t('agentResident.prompt')} · ${t(promptPreset.labelKey)}`
