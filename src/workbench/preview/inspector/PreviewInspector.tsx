...
export default function PreviewInspector({ timeline, collapsed, onToggleCollapsed }: PreviewInspectorProps): JSX.Element {
  ...
  if (collapsed) return <div className="flex h-full w-8 flex-col items-center border-l border-[var(--workbench-border)] bg-[var(--workbench-surface)] pt-2"><WorkbenchIconButton label={t('timelinePreview.previewInspector.expand')} title={t('timelinePreview.previewInspector.expand')} icon={<IconAdjustments size={16} />} onClick={onToggleCollapsed} /></div>
  return <aside className="flex h-full min-w-0 flex-col overflow-y-auto border-l border-[var(--workbench-border)] bg-[var(--workbench-surface)]" aria-label={t('timelinePreview.previewInspector.aria')}>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto">
      <div className="flex items-center gap-2 border-b border-[var(--workbench-border)] px-3 py-2" data-testid="preview-inspector-object">
        <span className={cn('h-6 w-1 rounded-pill', objectType === 'video' ? 'bg-[var(--workbench-video)]' : objectType === 'image' ? 'bg-[var(--workbench-image)]' : objectType === 'text' ? 'bg-[var(--workbench-text)]' : 'bg-[var(--workbench-accent)]')} />
        {objectType ? <TypeIcon type={objectType} /> : <IconMusic size={16} />}
        <div className="min-w-0"><div className="truncate text-caption font-semibold">{clip?.label || textClip?.text || t('timelinePreview.previewInspector.wholeFilm')}</div><div className="text-micro text-[var(--workbench-muted)]">{objectType ? t(({ video: 'timelinePreview.previewInspector.types.video', image: 'timelinePreview.previewInspector.types.image', text: 'timelinePreview.previewInspector.types.text' } as const)[objectType]) : t('timelinePreview.previewInspector.types.film')} · {clip ? `${((clip.endFrame - clip.startFrame) / fps).toFixed(1)}${t('timelinePreview.previewInspector.seconds')}` : t('timelinePreview.previewInspector.project')}</div></div>
      </div>
      {!objectType ? <>
        ...
        <Section title={t('timelinePreview.previewInspector.groups.text')}><Field label={t('timelinePreview.previewInspector.textStyle')}><TextClipStyleControls timeline={timeline} selectedTextClipId={textClip.id} /></Field></Section>
