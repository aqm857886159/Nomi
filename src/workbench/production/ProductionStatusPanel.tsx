...
export function ProductionStatusPanel({
  ...
  const handlePrimaryAction = React.useCallback(() => {
    ...
    Promise.resolve(onPrimaryAction(action)).finally(() => {
  ...
  return (
    ...
    >
      ...
      <figure
        ...
        className={cn(
          'm-0 overflow-hidden rounded-nomi border border-nomi-line-soft bg-nomi-ink-05',
          previewFocused && 'ring-2 ring-nomi-accent ring-offset-1',
        )}
      >
        {videoUrl ? (
          <video
        ...
        <figcaption className={cn('flex items-center justify-between gap-2 px-2.5 py-2 text-caption')}>
          <span className={cn('font-medium text-nomi-ink-80')}>
            {previewFocused
              ? t('generationCommon.production.runPanel.focusedArtifact')
              : t('generationCommon.production.runPanel.latestArtifact')}
          </span>
      ...
      <details className={cn('group border-t border-nomi-line-soft pt-2')}>
        <summary
          className={cn('flex cursor-pointer list-none items-center gap-1 text-caption font-medium text-nomi-ink-60')}
        >
          <IconChevronRight
            size={14}
            stroke={1.5}
