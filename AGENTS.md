...
| R14 | 周期审计 | ≥25 commit 或发版前：多维 subagent 审计 + 走查 + `docs/audit` 文档；**固定含 R14.1「同一语义有几份定义」横扫**（工具面/可见性口径/标识符/格式契约/字段取值来源/确认面/**规则的路径覆盖** 七维，机器只兜得住词表那一维；最难查的是 R14.1.a「一件事只做了一半、而做了的那一半看起来很完整」——先问「这条规则的对偶路径上有它吗」）|
