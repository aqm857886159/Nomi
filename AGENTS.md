...
> 真相源仍单一：规则索引 + 各处指针指明每条住哪，不另立第二份。改触发清单同步 `self-check.sh`，规则细节只改 L2。注：`.Codex/` 和 `.claude/` 被 gitignore，hook 不随 git 走，换机/新 worktree 需按所用工具复制对应 hooks 与 settings.json 的 hooks 块。
...
| `npx skills experimental_install` | 从 `skills-lock.json` 还原 `.Codex/skills/` 或 `.claude/skills/`（按所用工具，换机/协作者用） |
...
**P3 全绿 ≠ 完成** — CI 五门只证代码健康，证不了体验对不对。用户可见改动报完成前：① 和获批样张逐项并排对账；② 真体感走查（Playwright 截图人眼判断，不是 expect 断言）。缺一不算完成。**功能交付（尤其用户可见/体感）另过 R16：建几条「真实用户任务」端到端测试系统、带着真实任务跑通整个使用闭环、把过程中冒出的体验/设计/UI/UX/产品感/功能问题全修掉——才算真完成。**
...
| R16 | 真实任务测试系统=完成的一部分 | 功能交付必建真实用户任务端到端测试，带真实任务跑通闭环并修完过程中发现的体验、设计、UI、UX 和产品问题 |
