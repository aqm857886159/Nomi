/** Browser-safe synchronous SHA-256 over UTF-8 text. */
export function synchronousSha256(text: string): string {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h + bigSigma1 + choose + roundConstants[index] + words[index]) >>> 0
