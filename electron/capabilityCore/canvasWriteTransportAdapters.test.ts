...
async function setup(rawEvidence: unknown = RAW_EVIDENCE) {
  ...
  const write = vi.fn(async ({ receiptProposalId }: { receiptProposalId: string }) => ({
    applied: true,
...
describe("canvas.write Pi transport", () => {
  it("captures raw evidence before minting and dispatches one main-authorized write", async () => {
    ...
    expect(prepared?.invocation.input).toEqual({ operation: "set_node_prompt", nodeId: "client-alias", prompt: "new prompt" });
  ...
  it("rejects malformed raw evidence and mismatched approval authority before execute dispatch", async () => {
    ...
    const signal = new AbortController().signal;
    await expect(malformed.adapter.prepare({
      toolCallId: "tool-a", toolName: CANVAS_WRITE_ALIASES.setNodePrompt, args: { nodeId: "node-real", prompt: "new" },
    }, signal)).rejects.toMatchObject({ code: "capability_input_invalid" });

    const test = await setup();
    const prepared = await test.adapter.prepare({
      toolCallId: "tool-a", toolName: CANVAS_WRITE_ALIASES.setNodePrompt, args: { nodeId: "node-real", prompt: "new" },
    }, signal);
    await expect(test.adapter.execute(prepared!, {
      receiptProposalId: "receipt-a", approvalId: "approval-a", actionHash: "forged",
