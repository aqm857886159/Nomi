  ...
  try {
    ...
    const response = await runAgent(request, {
      ...
      awaitToolConfirmation: async (call, signal) => {
        ...
        // patch_shots 与 propose_storyboard_plan 同族：都只落创作 store 的分镜方案、
        // 由渲染层自行处理，不是画布写入（2026-09-03 新增 patch_shots 时补）。
        const isRendererHandledStoryboardProposal =
          canonicalCapability?.id === CANVAS_WRITE_CAPABILITY.id && ["propose_storyboard_plan", "patch_shots"].includes(call.toolName);
