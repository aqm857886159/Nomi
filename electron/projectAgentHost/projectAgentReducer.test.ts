...
describe("ProjectAgentHost reducer identity and exactly-once boundary", () => {
  it("requires explicit legacy provenance and keeps legacy threads read-only", () => {
    const legacy = {
      ...thread(),
      provenance: {
        ...
        source: {
          legacyArea: "creation" as const,
          legacySessionKey: "nomi:workbench:project-a:creation",
          legacyThreadId: "legacy-thread-a",
          sourceHash: "a".repeat(64),
        },
      },
    };
    const first = reduceProjectAgentMutation(createInitialProjectAgentState(binding), {
      commandId: "legacy-thread-put",
      expectedRevision: 0,
      binding,
      sender: { kind: "internal", senderId: "migration" },
      type: "thread.put",
      payload: { thread: legacy, makeActive: true },
    });

    expect(first.state.threads[0]?.provenance).toEqual(legacy.provenance);
    expect(() =>
      reduceProjectAgentMutation(first.state, {
        commandId: "legacy-thread-rewrite",
        expectedRevision: 1,
        binding,
        sender: { kind: "renderer", senderId: "renderer-a" },
        type: "thread.put",
        payload: { thread: { ...legacy, title: "rewrite" } },
      }),
    ).toThrowError(expect.objectContaining<Partial<ProjectAgentReducerError>>({ code: "thread_read_only" }));
  });

  it("does not replace a legacy thread during enqueue", () => {
    const legacy = {
      ...thread(),
      provenance: {
        kind: "legacy" as const,
    ...
    const first = reduceProjectAgentMutation(createInitialProjectAgentState(binding), {
      commandId: "legacy-thread-seed-for-enqueue",
      expectedRevision: 0,
      binding,
      sender: { kind: "internal", senderId: "migration" },
      type: "thread.put",
    ...
    expect(() =>
      reduceProjectAgentMutation(first.state, {
        ...enqueueMutation("legacy-enqueue", "turn-a"),
        expectedRevision: 1,
      }),
    ).toThrowError(expect.objectContaining<Partial<ProjectAgentReducerError>>({ code: "thread_read_only" }));
  });

  it("does not let a renderer mint legacy provenance", () => {
    const canonical = reduceProjectAgentMutation(createInitialProjectAgentState(binding), {
      commandId: "canonical-thread-for-provenance",
      expectedRevision: 0,
      binding,
      sender: { kind: "renderer", senderId: "renderer-a" },
      type: "thread.put",
      payload: { thread: thread(), makeActive: true },
    });
    const legacy = {
      ...thread(),
      provenance: {
    ...
    expect(() =>
      reduceProjectAgentMutation(canonical.state, {
        commandId: "renderer-mints-legacy",
        expectedRevision: 1,
        binding,
        sender: { kind: "renderer", senderId: "renderer-a" },
        type: "thread.put",
        payload: { thread: legacy },
      }),
    ).toThrowError(expect.objectContaining<Partial<ProjectAgentReducerError>>({ code: "thread_read_only" }));
  });

  it("does not let migration downgrade an existing canonical thread to legacy", () => {
    const canonical = reduceProjectAgentMutation(createInitialProjectAgentState(binding), {
      commandId: "canonical-thread-for-migration",
      expectedRevision: 0,
      binding,
      sender: { kind: "renderer", senderId: "renderer-a" },
      type: "thread.put",
    ...
    expect(() =>
      reduceProjectAgentMutation(canonical.state, {
        commandId: "migration-downgrade-canonical",
        expectedRevision: 1,
        binding,
        sender: { kind: "internal", senderId: "migration" },
        type: "thread.put",
  ...
  it("does not allow migration to downgrade an existing canonical thread to legacy", () => {
    const canonical = reduceProjectAgentMutation(createInitialProjectAgentState(binding), {
      commandId: "canonical-thread-for-downgrade",
      expectedRevision: 0,
      binding,
      sender: { kind: "renderer", senderId: "renderer-a" },
      type: "thread.put",
    ...
    expect(() =>
      reduceProjectAgentMutation(canonical.state, {
        commandId: "migration-downgrades-canonical",
        expectedRevision: 1,
        binding,
        sender: { kind: "internal", senderId: "migration" },
        type: "thread.put",
  ...
  it("deletes a legacy archive thread and its records in one removal patch", () => {
    const legacy = {
      ...thread(),
      provenance: {
        kind: "legacy" as const,
        readOnly: true as const,
        source: {
          legacyArea: "generation" as const,
          legacySessionKey: "nomi:workbench:project-a:generation",
          legacyThreadId: "legacy-thread-a",
          sourceHash: "b".repeat(64),
        },
      },
    };
    const legacyTurn = { ...turn(), status: "done" as const };
    const legacyUser = userItem();
    const legacyAssistant = { ...assistantItem(), status: "done" as const };
    const legacyQueue = { ...queueItem(), status: "done" as const };
    const seeded = snapshotProjectAgentHostState({
      binding,
      hostRevision: 0,
      commandLedgerHighWater: 0,
      activeThreadId: legacy.threadId,
      threads: [legacy],
      turns: [legacyTurn],
      items: [legacyUser, legacyAssistant],
      queue: [legacyQueue],
      proposalApprovals: [],
      recentAppliedCommands: [],
    });
    const removed = reduceProjectAgentMutation(seeded, {
      commandId: "legacy-thread-remove",
      expectedRevision: 0,
      binding,
      sender: { kind: "renderer", senderId: "renderer-a" },
      type: "thread.remove",
      payload: { threadId: legacy.threadId, occurredAt: now },
    });

    expect(removed.state.threads).toEqual([]);
    expect(removed.state.turns).toEqual([]);
    expect(removed.state.items).toEqual([]);
    expect(removed.state.queue).toEqual([]);
    expect(removed.state.activeThreadId).toBeNull();
    if (!removed.patch) throw new Error("legacy thread removal did not emit a patch");
    expect(removed.patch.changes).toEqual([
      { kind: "item-removed", itemId: legacyUser.itemId },
      { kind: "item-removed", itemId: legacyAssistant.itemId },
      { kind: "queue-removed", queueItemId: legacyQueue.queueItemId },
      { kind: "turn-removed", turnId: legacyTurn.turnId },
      { kind: "thread-removed", threadId: legacy.threadId },
      { kind: "active-thread-changed", activeThreadId: null },
    ]);
  });

  it("does not delete a legacy thread while a linked turn is still queue-blocking", () => {
    const legacy = {
      ...thread(),
      provenance: {
        kind: "legacy" as const,
        ...
        source: {
          legacyArea: "generation" as const,
          legacySessionKey: "nomi:workbench:project-a:generation",
          legacyThreadId: "legacy-thread-live",
          sourceHash: "f".repeat(64),
        },
      },
    };
    const queuedTurn = turn();
    const queuedUser = userItem();
    const queuedQueue = queueItem();
    const seeded = snapshotProjectAgentHostState({
      binding,
      hostRevision: 0,
      commandLedgerHighWater: 0,
    ...
    expect(() =>
      reduceProjectAgentMutation(seeded, {
        commandId: "legacy-live-thread-remove",
        expectedRevision: 0,
        binding,
        sender: { kind: "renderer", senderId: "renderer-a" },
        type: "thread.remove",
