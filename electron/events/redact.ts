...
//
// 与 `electron/logging/redact.ts` 的分工(别把两者混成一件事):这份处理**事件负载**——
// 任意深度的对象,判据是「拿已知密钥值精确匹配 + 常见密钥形态兜底」,产出仍是对象;
// 那份处理**一行日志文本**,判据是「字段名黑名单 + 值形态」,产出是一行可 grep 的串。
// 输入形状、判据和产物都不同,合并只会让两边都变钝。
