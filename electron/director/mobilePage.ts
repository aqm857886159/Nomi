...
.right{display:flex;flex-direction:column;gap:10px;padding:12px}
...
(function(){
  ...
  function connect(){
    ...
    ws.onclose = function(){ if (disposed || ws !== socket) return; connected = false; clearPreview(); recBtn.disabled = true; setStatus(T.disconnected || 'disconnected', false); reconnectTimer = setTimeout(connect, 1500) };
    ws.onmessage = function(ev){
      ...
      try {
        ...
        if (m && m.type === 'ping') ws.send(JSON.stringify({ type: 'pong', t: m.t }));
