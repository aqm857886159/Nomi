...
const shouldOpenDevTools =
  isDev && (process.env.NOMI_OPEN_DEVTOOLS === "1" || process.env.NOMI_DESKTOP_OPEN_DEVTOOLS === "1");
...
function removeHeader(headers: Record<string, string[] | string | undefined>, name: string): void {
...
function installContentSecurityPolicy(targetSession: Electron.Session): void {
  ...
  const crossOriginIsolationDisabled =
    lowMemoryMode || SKIP_CROSS_ORIGIN_ISOLATION || process.env.NOMI_DISABLE_CROSS_ORIGIN_ISOLATION === "1";
