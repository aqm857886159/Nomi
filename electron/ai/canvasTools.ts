...
const storyboardShotSchema = z.object({
  ...
  modelVendor: z.string().optional().describe("Provider paired with modelKey, chosen from the same row in the available-models list."),
  ...
  keyframe: z
    .object({
      ...
      modelVendor: z.string().optional().describe("Provider paired with the first-frame modelKey, from the same available-models row."),
...
export const canvasTools = {
  ...
  create_camera_move: tool({
    description:
      ...
      "Tiered rule: the `move` enum is the PRECISE first choice (deterministic 3D camera-path render). If the intended move is OUTSIDE the enum (whip-pan, handheld follow, a compound/sequenced move, or 'match this reference video'), do NOT force the nearest wrong enum — leave move empty and use customMove (prompt-guided into the video node's prompt, no 3D render, honest about lower fidelity). Never force-map a clearly-different intent into a wrong enum value.\n" +
