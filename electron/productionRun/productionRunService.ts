...
type SafeProductionGate = Omit<ProductionRun['gates'][number], 'planHash' | 'jobIds' | 'contract'> & {
  contract?: ReturnType<typeof safeContract>
}
...
function safeExternalText(value: string): string {
...
function safeContract(contract: ProductionRun['gates'][number]['contract']) {
...
function safeRunProjection(run: ProductionRun): Omit<ProductionRunProjection, 'artifacts' | 'openInNomi'> {
  return {
    ...
    gates: run.gates.map((gate) => ({
      ...
      ...(gate.contract ? { contract: safeContract(gate.contract) } : {}),
    })),
