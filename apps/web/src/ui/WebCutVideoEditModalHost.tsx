...
type WebCutCancelMessage = {
  type: 'webcut:cancel'
  requestId: string
}

type WebCutMessage = WebCutExportMessage | WebCutCancelMessage

function isWebCutMessage(input: unknown): input is WebCutMessage {
  if (!input || typeof input !== 'object' || Array.isArray(input)) return false
  const record = input as { type?: unknown; requestId?: unknown; mime?: unknown; buffer?: unknown }
  if (record.type !== 'webcut:export' && record.type !== 'webcut:cancel') return false
  if (typeof record.requestId !== 'string') return false
  if (record.type === 'webcut:cancel') return true
  return record.mime === 'video/mp4' && record.buffer instanceof ArrayBuffer
}
...
export function WebCutVideoEditModalHost(): JSX.Element | null {
  ...
  const iframeSrc = React.useMemo(() => {
    ...
    const proxiedVideoUrl = isLocalWebCutBase(baseUrl)
      ? buildVideoPlaybackUrl(payload.videoUrl).replace(/^\/api\/assets\//, '/assets/')
      : buildVideoPlaybackUrl(payload.videoUrl)
    const tapToken =
      token ||
      getAuthToken() ||
    ...
    const onMessage = async (e: MessageEvent) => {
      ...
      try {
        ...
        try {
          ...
          const msg = getErrorMessage(err, '剪辑结果应用失败')
          toast(msg, 'error')
        } finally {
          payload?.onClose?.()
          close()
