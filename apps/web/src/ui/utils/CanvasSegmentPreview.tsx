...
function canCancelVideoFrameCallback(video: HTMLVideoElement): video is VideoFrameCallbackCapable {
  return typeof (video as { cancelVideoFrameCallback?: unknown }).cancelVideoFrameCallback === 'function'
}
...
export function CanvasSegmentPreview(props: CanvasSegmentPreviewProps): JSX.Element {
  ...
  const stopLoops = React.useCallback(() => {
    ...
    rafRef.current = null
    if (frameReqRef.current != null && videoRef.current && canCancelVideoFrameCallback(videoRef.current)) {
      try {
        videoRef.current.cancelVideoFrameCallback(frameReqRef.current)
      } catch {
        // ignore
      }
    }
