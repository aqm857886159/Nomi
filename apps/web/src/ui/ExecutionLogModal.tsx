...
import { Stack, Group, Text, ScrollArea, Table, Divider, Tooltip } from '@mantine/core'
import { IconCopy, IconFilter, IconX } from '@tabler/icons-react'
import { API_BASE } from '../api/httpClient'
import { getWorkflowExecution, listWorkflowNodeRuns, type WorkflowExecutionDto, type WorkflowExecutionEventDto, type WorkflowNodeRunDto } from '../api/server'
import { getAuthToken, getAuthTokenFromCookie } from '../auth/store'
import { DesignBadge, DesignButton, DesignModal, IconActionButton } from '../design'

type NomiFocusWindow = Window & {
...
export function ExecutionLogModal(props: {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setStatusLine(errorMessage(err))
      }
    })()
  ...
  const focusNode = React.useCallback((nodeId: string) => {
    try {
      const focusWindow = window as NomiFocusWindow
      focusWindow.__tcFocusNode?.(nodeId)
    } catch {
      // ignore
    }
  }, [])
  ...
  return (
    <DesignModal className={modalClassName} opened={opened} onClose={onClose} title="运行日志" centered size="lg">
      <Stack className="execution-log-body" gap="sm">
          ...
          <Group className="execution-log-controls" gap="xs" wrap="nowrap">
            <Tooltip className="execution-log-filter-tooltip" label={onlyIssues ? '只看告警/错误（已开启）' : '只看告警/错误'}>
              <IconActionButton
                ...
                icon={<IconFilter className="execution-log-filter-icon" size={14} />}
              />
            </Tooltip>
            <Tooltip className="execution-log-node-filter-tooltip" label={filterNodeId ? '清除节点筛选' : '未筛选节点'}>
              <IconActionButton
                ...
                icon={<IconX className="execution-log-node-filter-icon" size={14} />}
              />
            </Tooltip>
            <DesignBadge className="execution-log-status-line">{statusLine}</DesignBadge>
          </Group>
        </Group>
          ...
          <>
            <Group className="execution-log-summary" justify="space-between">
              <Group className="execution-log-summary-left" gap="xs">
                ...
                <DesignBadge className="execution-log-summary-total" size="xs">
                  {runsSummary.total}
                </DesignBadge>
                  ...
                  {nodeRuns.map((r) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Table.Td className="execution-log-runs-cell">
                          <DesignBadge className="execution-log-runs-status" size="xs" color={color}>
                            {r.status}
                          </DesignBadge>
                        </Table.Td>
                        <Table.Td className="execution-log-runs-cell">
              ...
              {visibleEvents.map((e) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Table.Td className="execution-log-events-cell">
                      <DesignBadge className="execution-log-events-level" size="xs" color={levelColor}>
                        {e.level}
                      </DesignBadge>
                    </Table.Td>
                    <Table.Td className="execution-log-events-cell">
                    ...
                    <Table.Td className="execution-log-events-cell">
                      <Tooltip className="execution-log-events-copy-tooltip" label="复制">
                        <IconActionButton
                          ...
                          icon={<IconCopy className="execution-log-events-copy-icon" size={14} />}
                        />
                      </Tooltip>
                    </Table.Td>
                  </Table.Tr>
        ...
        <Group className="execution-log-footer" justify="flex-end">
          <DesignButton className="execution-log-close" variant="subtle" onClick={onClose}>
            关闭
          </DesignButton>
        </Group>
      </Stack>
