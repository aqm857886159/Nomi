...
function readUrlResultAt(results: Record<string, unknown>[], index: number): UrlResultRecord | null {
  const item = results[index]
  return item ? { url: item.url, thumbnailUrl: item.thumbnailUrl } : null
}

function findFirstResultWithUrl(results: Record<string, unknown>[]): UrlResultRecord | null {
  return results.find((result) => readString(result.url).length > 0) ?? null
}
      ...
      <div className="tapshow-card-media">
        ...
        <div className="tapshow-card-overlay">
          <Group className="tapshow-card-badges" gap={8}>
            ...
            {asset.modelKey && (
              <DesignBadge className="tapshow-card-badge" size="xs" radius="md" variant="outline" color="gray">
                {asset.modelKey}
              </DesignBadge>
            )}
            {durationText && (
              <DesignBadge className="tapshow-card-badge" size="xs" radius="md" variant="filled" color="dark">
                {durationText}
              </DesignBadge>
            )}
          </Group>
...
function pickCoverFromFlows(flows: FlowDto[]): ProjectCover | null {
  ...
  const scan = (kind: 'video' | 'image'): ProjectCover | null => {
    for (const f of sorted) {
      const data = isRecord(f.data) ? f.data : {}
      const nodes = readRecordArray(data.nodes)
      for (let i = nodes.length - 1; i >= 0; i -= 1) {
        const node = nodes[i]
        const nd = isRecord(node.data) ? node.data : {}

        if (kind === 'video') {
          const results = readRecordArray(nd.videoResults)
          const primaryIndex = readFiniteIndex(nd.videoPrimaryIndex)
          const primary = readUrlResultAt(results, primaryIndex)
          const fallback = findFirstResultWithUrl(results)
          const videoUrl = readString(primary?.url) || readString(fallback?.url) || readString(nd.videoUrl)
          if (!videoUrl) continue
          const thumbnailUrl = readString(primary?.thumbnailUrl) || readString(fallback?.thumbnailUrl) || readString(nd.videoThumbnailUrl) || readString(nd.thumbnailUrl) || null
          return { type: 'video', url: videoUrl, thumbnailUrl, flowId: String(f.id) }
        }

        const imageResults = readRecordArray(nd.imageResults)
        const imagePrimaryIndex = readFiniteIndex(nd.imagePrimaryIndex)
        const primaryImg = readUrlResultAt(imageResults, imagePrimaryIndex)
        const fallbackImg = findFirstResultWithUrl(imageResults)
        const imageUrl = readString(nd.imageUrl) || readString(primaryImg?.url) || readString(fallbackImg?.url)
...
function TapshowFullPageInner({ className }: { className?: string }): JSX.Element {
  ...
  const reloadProjects = React.useCallback(async (opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      console.error(err)
      toast(err instanceof Error ? err.message : '加载公开项目失败', 'error')
      setPublicProjects([])
    } finally {
  ...
  const reloadAssets = React.useCallback(
    async (opts?: { silent?: boolean }) => {
      ...
      try {
        ...
        console.error(err)
        toast(err instanceof Error ? err.message : '加载 TapShow 作品失败', 'error')
      } finally {
        setLoading(false)
        setRefreshing(false)
      }
            ...
            <Group className="tapshow-fullpage-header-right" gap={6}>
              {isAdmin && (
                <Tooltip className="tapshow-fullpage-admin-tooltip" label="看板（仅管理员）" withArrow>
                  <IconActionButton
                    ...
                    size="sm"
                    variant="subtle"
                    icon={<IconChartBar className="tapshow-fullpage-admin-icon" size={14} />}
                    aria-label="看板"
                    onClick={() => {
                      try {
                        const url = new URL(window.location.href)
              ...
              <Tooltip className="tapshow-fullpage-refresh-tooltip" label="刷新" withArrow>
                <IconActionButton
                  ...
                  size="sm"
                  variant="subtle"
                  icon={<IconRefresh className="tapshow-fullpage-refresh-icon" size={14} />}
          ...
          <Stack className="tapshow-fullpage-projects" gap={8} mb="lg">
            <Group className="tapshow-fullpage-projects-header" justify="space-between" align="center" wrap="wrap" gap={8}>
              <Group className="tapshow-fullpage-projects-title" gap={8} align="center">
                ...
                <DesignBadge className="tapshow-fullpage-projects-count" variant="light" color="gray">
              ...
              <Group className="tapshow-fullpage-projects-actions" gap={6}>
                ...
                <Tooltip className="tapshow-fullpage-projects-refresh-tooltip" label="刷新公开项目" withArrow>
                  <IconActionButton
                    ...
                    size="sm"
                    variant="light"
                    icon={<IconRefresh className="tapshow-fullpage-projects-refresh-icon" size={14} />}
                    aria-label="刷新公开项目"
                    onClick={() => {
          ...
          <Group className="tapshow-fullpage-filters" justify="space-between" align="center" mb="md" wrap="wrap" gap={8}>
            <Group className="tapshow-fullpage-filters-left" gap={8} wrap="wrap">
              ...
              <IconActionButton
                ...
                radius="md"
                variant="light"
                icon={sortOrder === 'desc' ? <IconSortDescending className="tapshow-fullpage-filter-order-icon" size={14} /> : <IconFilter className="tapshow-fullpage-filter-order-icon" size={14} />}
                aria-label="切换排序方向"
      ...
      >
        <Stack className="tapshow-fullpage-prompt-modal-stack" gap="sm">
          <Group className="tapshow-fullpage-prompt-modal-actions" justify="flex-end">
            <DesignButton className="tapshow-fullpage-prompt-modal-copy" size="xs" variant="light" leftSection={<IconCopy size={14} />} onClick={() => { void handleCopyPrompt() }}>
              复制提示词
            </DesignButton>
          </Group>
