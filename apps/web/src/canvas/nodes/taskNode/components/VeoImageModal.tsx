...
import { DesignButton, IconActionButton, InlinePanel } from '../../../../design'
    ...
    >
      <Stack className="veo-image-modal-body" gap="sm">
              ...
              <Group className="veo-image-modal-reference-list" gap={6} wrap="wrap">
                {veoReferenceImages.map((url) => (
                  <InlinePanel className="veo-image-modal-reference-card" key={url} padding="compact" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <IconActionButton
                      className="veo-image-modal-reference-remove"
                      size="xs"
                      variant="subtle"
                      onClick={() => onRemoveReferenceImage(url)}
                      icon={<IconTrash className="veo-image-modal-reference-remove-icon" size={12} />}
                    />
                  </InlinePanel>
                ))}
              </Group>
        ...
        {mode === 'first' && trimmedFirstFrameUrl && (
          <DesignButton className="veo-image-modal-clear" variant="subtle" size="xs" onClick={() => onSetFirstFrameUrl('')}>
            清除首帧
          </DesignButton>
        )}
        {mode === 'last' && trimmedLastFrameUrl && (
          <DesignButton className="veo-image-modal-clear" variant="subtle" size="xs" onClick={() => onSetLastFrameUrl('')} disabled={!firstFrameLocked}>
            清除尾帧
          </DesignButton>
        )}
      </Stack>
