...
function checkGeometry(elem, actualRect, anchor, specRef, injectedFaults = {}) {
  ...
  const tol = elem.tolerances || { w: 2, h: 2, x: 2, y: 2 }
  ...
  if (Math.abs(actualRect.w - faultyExpected.w) > wTol) {
    fail(anchor, specRef, `宽度不符: 期望 ${faultyExpected.w}px±${wTol}，实测 ${actualRect.w}px（差 ${actualRect.w - faultyExpected.w > 0 ? '+' : ''}${actualRect.w - faultyExpected.w}px）`)
  } else {
    pass(anchor, specRef, `宽度 ${actualRect.w}px ✓`)
  }

  if (Math.abs(actualRect.h - faultyExpected.h) > hTol) {
    fail(anchor, specRef, `高度不符: 期望 ${faultyExpected.h}px±${hTol}，实测 ${actualRect.h}px（差 ${actualRect.h - faultyExpected.h > 0 ? '+' : ''}${actualRect.h - faultyExpected.h}px）`)
  } else {
