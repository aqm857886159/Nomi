...
const FIRST_CALLS: ReadonlyArray<{ label: string; tool: string; args: unknown }> = [
  { label: 'well-formed', tool: 'write_script', args: { content: APPENDED, where: 'end' } },
  { label: 'whole-argument-object serialized as a JSON string', tool: 'write_script', args: JSON.stringify({ content: APPENDED, where: 'end' }) },
  { label: 'field named `text` instead of `content`', tool: 'write_script', args: { text: APPENDED, where: 'end' } },
  { label: 'field named `body` instead of `content`', tool: 'write_script', args: { body: APPENDED, where: 'end' } },
  { label: 'content split into an array of strings', tool: 'write_script', args: { content: ['The lights ', 'went out.'], where: 'end' } },
  { label: 'read tool handed the argument a sibling write tool takes', tool: 'read_script', args: { where: 'end' } },
...
async function measure(t: TestContext, arm: MeasurementArm): Promise<Measurement> {
  ...
  for (const attempt of FIRST_CALLS) {
    const replies: FixtureReply[] = [
      ...
      // 一次写对时它不会被消费（夹具按需出队）。
      { type: 'tool', calls: [{ id: 'second', name: 'write_script', arguments: { content: APPENDED, where: 'end' } }] },
      { type: 'text', text: 'Done.' },
    ];
