...
test('C59 on-demand models returns every mode contract and follows the latest catalog', async () => {
  ...
  entries = [];
  assert.deepEqual(JSON.parse((await tool.execute('new', {})).content[0]!.text), { models: [] });
  assert.deepEqual(JSON.parse((await tool.execute('invalid', { kind: 'audio' })).content[0]!.text), { models: [] });
});
