...
5. Count the doors before choosing where to fix. Run `node scripts/door-map.mjs <mutator symbol or file>` to enumerate every write and read entry point of the state whose invariant broke, and paste the output into the contract's `doors`. A door map you typed by hand is the same thing as the sentence "I checked"; the script is the check. The tool counts direct call sites only, so follow one more hop when a door is reached through a wrapper.
6. Search the repository by data shape, contract, producer, and consumer. Record at least two independently checked same-class entry points; do not search only for the reported provider, version, model, or fixture name.
7. Identify the earliest shared boundary that can own the invariant. If callers do not converge, first consolidate ownership or define multiple explicit shared boundaries.
8. For third-party behavior, inspect current official documentation, source, installed versions, and platform/package drift. Choose `upgrade-now`, `retain-with-exit`, or `not-applicable`; retention requires a target and testable exit criteria.
9. Read `references/contract-v3.template.json`, then create or update a schema-v3 `docs/fixes/*.root-cause.json` before a recurring or high-risk production fix. Preserve the template's exact field names, object shapes, and enums; every changed high-risk production file must be covered by `scope_paths`.
10. Add the failing reported-case test and at least one class-level test. Run only this red slice before implementation.
11. For recurring repairs, enforce the invariant at the shared boundary and list changed structural code in `prevention.artifacts`. Delete obsolete behavior in the same change; tests, documentation, and fallback paths are not structural prevention.
12. Run the changed class tests and `pnpm run check:root-cause-contracts`. After the logical batch is stable, run the risk-selected repository validation once.
...
- `doors`: every write and read entry point of the affected state, generated by `scripts/door-map.mjs`, each `{kind, path, line, symbol}` resolving to that line;
- `door_reduction`: `{before, after}` where `after` equals `doors.length`; when two or more doors survive unchanged, `why_not` must say why each entry has to exist;
...
1. Can the same class still enter through another caller, provider, version, platform, or old stored value? Answer from the door map, not from memory.
