...
// （tests/ux/fixtures/real-narration.mp3）+ 一张真实图片参考（resources/onboarding-demo/kid.jpg）。
...
//
// 2026-09-11 根因已定位并修复（诊断分支 diag/audio-paid-smoke-20260911）：此前 5 次失败
// （code=task_failed / cost=0）的根因不是音频链路或本产品代码，是**这个脚本自己用的图片夹具**——
// tests/ux/fixtures/test-upload.png 是一张 100×100 单色占位图（`file` 一测即现），APIMart 自己的
// 上传端点对它直接判 `400 invalid image content`；代码按既定设计回落到 KIE 通用文件host拿到一个
// 能正常访问的 https URL，所以出站抓包里"链路看着通"，但那张退化占位图本身仍然会让 Seedance 2.0
// 的实际生成判 task_failed。换成一张真实照片（resources/onboarding-demo/kid.jpg，720×720）后，
// 同一条链路——图生视频、以及本脚本这种图+音频 omni 参考——均真实出片成功。详见
// docs/lessons/paid-smoke-apimart-only.md。
...
// 真实照片而非 tests/ux/fixtures/test-upload.png——后者是 100×100 单色占位图，会被 APIMart
// 判定 invalid image content 并一路吞成通用 task_failed（根因见上方注释与教训文件）。
