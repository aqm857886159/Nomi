...
/**
 * 轴 C：doc-only 放行的判据必须**认得非 ASCII 路径**（2026-09-07 实测栽过）。
 *
...
export const AXES = ['writer', 'stamp', 'push-commands', 'docs-only', 'secret-guard', 'suggestion']
  ...
  if (problems.length === 0) {
    console.log(
      `✓ Bash 闸门行为契约通过：戳契约（gates 写、hook 读，同一个 ${MARKER_BASENAME}）` +
        ` + pre-push 与 secret-guard 的命令识别矩阵 + doc-only 判据（含非 ASCII 路径），均已实跑 hook 验证。`,
    )
    process.exit(0)
  }
