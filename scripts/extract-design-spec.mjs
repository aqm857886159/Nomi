...
const specAnchors = [
  ...
  'data-agent-composer-mode', 'data-agent-composer-model', 'data-agent-composer-skill',
...
// ── 件2：同时生成 .auto.mjs 契约文件（与意图层同目录/同格式/同被门岗扫描）─────────
// agent-ui-spec.generated.json 是中间产物（机器提取数据），.auto.mjs 才是契约本体
// （被 assertMockupContract 消费、被 check:mockup-contracts 门岗引用）。
const mockupName = path.basename(MOCKUP_PATH, '.html')
const AUTO_CONTRACT_DIR = path.join(ROOT, 'docs', 'design', 'mockups', 'contracts')
const AUTO_CONTRACT_PATH = path.join(AUTO_CONTRACT_DIR, `${mockupName}.auto.mjs`)

// 将 JSON 规格表翻译成 _contract.mjs 的 geometry 规则数组（selector 用 cssSelector）。
// 容差策略统一为 max(4px步进, 25%)，规格表里的 tolerances 字段仅供人工参考，不再做容差上限。
const autoGeometry = allElements.map(elem => ({
  name: `[${elem.specRef}] ${elem.desc}（${elem.anchor}）`,
...
// 样张 hash：${mockupHash}（hash 不变代表样张未改，规格仍有效）
//
...
export default {
  ...
  surface: 'Agent UI（自动层·${allElements.length} 条规格/${screens.length} 屏）',
  layer: 'auto',

  geometry: ${JSON.stringify(autoGeometry, null, 2)},
}
`

fs.mkdirSync(AUTO_CONTRACT_DIR, { recursive: true })
fs.writeFileSync(AUTO_CONTRACT_PATH, autoContractContent, 'utf8')
...
console.log(`   · 自动层契约：${path.relative(ROOT, AUTO_CONTRACT_PATH)}（${autoGeometry.length} 条几何规则）`)
