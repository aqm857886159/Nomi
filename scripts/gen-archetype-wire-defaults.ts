...
const WIRE_DEFAULTS_TYPE = "Record<string, Record<string, Record<string, Record<string, unknown>>>>";

export function renderGeneratedFile(defaults: WireDefaults): string {
  return (
    `${BANNER}import { REPLICATE_ARCHETYPE_WIRE_DEFAULTS } from "./archetypeWireDefaults.replicate.generated";\n` +
    `// 档案参数默认值桥接：headless/MCP 生成缺参时 runtime 按 (archetypeId, taskKind) 兜底填，既有值优先。\n` +
    `const CORE_ARCHETYPE_WIRE_DEFAULTS: ${WIRE_DEFAULTS_TYPE} = ${JSON.stringify(defaults, null, 2)};\n` +
    `export const ARCHETYPE_WIRE_DEFAULTS: ${WIRE_DEFAULTS_TYPE} = {\n` +
    `  ...CORE_ARCHETYPE_WIRE_DEFAULTS,\n` +
    `  ...REPLICATE_ARCHETYPE_WIRE_DEFAULTS,\n` +
    `};\n`
  );
}

export function renderReplicateDefaultsFile(defaults: WireDefaults): string {
  return (
    `${BANNER}// Replicate 分片：避免默认值主表超过 800 行文件体积门岗。运行时由主表透明合并。\n` +
    `export const REPLICATE_ARCHETYPE_WIRE_DEFAULTS: ${WIRE_DEFAULTS_TYPE} = ${JSON.stringify(defaults, null, 2)};\n`
  );
}
...
if (isMain) {
  ...
  const allDefaults = buildArchetypeWireDefaults();
  const coreDefaults: WireDefaults = {};
  const replicateDefaults: WireDefaults = {};
  for (const [archetypeId, value] of Object.entries(allDefaults)) {
    (archetypeId.startsWith("replicate-") ? replicateDefaults : coreDefaults)[archetypeId] = value;
  }
  const outputs: Array<{ file: string; content: string }> = [
    { file: "archetypeWireDefaults.generated.ts", content: renderGeneratedFile(coreDefaults) },
    { file: "archetypeWireDefaults.replicate.generated.ts", content: renderReplicateDefaultsFile(replicateDefaults) },
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log(`wrote ${outputs.map((o) => o.file).join(", ")} (${Object.keys(allDefaults).length} archetypes)`);
  }
}
