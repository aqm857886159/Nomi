...
> 真相源仍单一：规则索引 + 各处指针指明每条住哪，不另立第二份。改触发清单同步 `self-check.sh`，规则细节只改 L2。注：`.claude/` 仍被 gitignore，但 **hook 本身已纳入版本库**——单一真相源是 `scripts/claude-hooks/`（脚本 + settings 的 hooks 块），`pnpm install` 的 postinstall 自动装进 `.claude/`，换机/新 worktree **不再需要手动复制**。本地改了 hook 要同步回 `scripts/claude-hooks/` 并提交，否则 `check:claude-hooks` 报漂移（它们是 R11/R25/self-check 的执行体，漂了等于闸门失真）。`.claude/settings.json` 的 `env`/`permissions` 与 `violations.log` 仍是机器本地的，安装器不碰。**在途的老 worktree 先跑一次 `pnpm install`**：2026-09-02 起 push 闸的五门戳改成**一树一戳**（`$GIT_DIR/nomi-gates-ok`，带盖戳时的 HEAD + 30 分钟 TTL，由 `pnpm run gates` 末尾的 `scripts/stamp-gates-ok.mjs` 盖），没装新 hook 的树还在认主仓固定路径的旧戳，会拿别棵树的戳**放行没过门的 push**（详见 `docs/lessons/`）。
